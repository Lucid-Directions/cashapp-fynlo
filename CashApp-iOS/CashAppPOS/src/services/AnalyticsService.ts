...
class AnalyticsService {
  ...
  async getAnalyticsData(period: 'today' | 'week' | 'month' | 'year' = 'month'): Promise<AnalyticsData> {
  ...
  async getRevenueAnalytics(period: 'today' | 'week' | 'month' | 'year' = 'month'): Promise<RevenueData> {
    const data = await this.getAnalyticsData(period);
    return data.revenue;
  }

  async getTransactionAnalytics(period: 'today' | 'week' | 'month' | 'year' = 'month'): Promise<TransactionData> {
    const data = await this.getAnalyticsData(period);
  ...
  async getTrendAnalytics(period: 'today' | 'week' | 'month' | 'year' = 'month'): Promise<TrendData> {
    const data = await this.getAnalyticsData(period);
    return data.trends;
  }

  async exportData(format: 'csv' | 'json' | 'pdf', period: string): Promise<{ url: string; filename: string }> {
  ...
  private getPeriodMultiplier(period: 'today' | 'week' | 'month' | 'year'): number {
    switch (period) {
      case 'today': return 0.033; // 1/30th of month
      case 'week': return 0.23; // ~1/4 of month
      case 'month': return 1;
      case 'year': return 12;
      default: return 1;
    }
  ...
  private generateRevenueData(multiplier: number, period: string): RevenueData {
    ...
    return {
      ...
      byRestaurant: [
        { restaurantId: '1', name: 'Fynlo Coffee Shop', revenue: Math.round(total * 0.36), commission: Math.round(total * 0.36 * 0.025), growth: 15.2 },
        { restaurantId: '2', name: 'Fynlo Burger Bar', revenue: Math.round(total * 0.31), commission: Math.round(total * 0.31 * 0.027), growth: 8.7 },
        { restaurantId: '3', name: 'Fynlo Pizza Palace', revenue: Math.round(total * 0.26), commission: Math.round(total * 0.26 * 0.024), growth: 5.3 },
        { restaurantId: '4', name: 'Fynlo Taco Stand', revenue: Math.round(total * 0.07), commission: Math.round(total * 0.07 * 0.028), growth: -2.1 },
      ],
    };
  }
  ...
  private generateTransactionData(multiplier: number, period: string): TransactionData {
    ...
    return {
      ...
      byPeriod: this.generatePeriodData(period, total),
      byPaymentMethod: [
        { method: 'Card', count: Math.round(total * 0.65), value: Math.round(total * 0.65 * avgValue), percentage: 65 },
        { method: 'Contactless', count: Math.round(total * 0.25), value: Math.round(total * 0.25 * avgValue), percentage: 25 },
        { method: 'Mobile Pay', count: Math.round(total * 0.08), value: Math.round(total * 0.08 * avgValue), percentage: 8 },
        { method: 'Cash', count: Math.round(total * 0.02), value: Math.round(total * 0.02 * avgValue), percentage: 2 },
      ],
    };
  }
  ...
  private generatePerformanceData(): PerformanceData {
    return {
      rankings: [
        { rank: 1, restaurantId: '1', name: 'Fynlo Coffee Shop', revenue: 45200, growth: 15.2, score: 92 },
        { rank: 2, restaurantId: '2', name: 'Fynlo Burger Bar', revenue: 38900, growth: 8.7, score: 87 },
        { rank: 3, restaurantId: '3', name: 'Fynlo Pizza Palace', revenue: 32800, growth: 5.3, score: 81 },
        { rank: 4, restaurantId: '4', name: 'Fynlo Taco Stand', revenue: 8500, growth: -2.1, score: 73 },
      ],
      metrics: [
        { restaurantId: '1', name: 'Fynlo Coffee Shop', avgOrderValue: 12.50, transactionCount: 3616, customerSatisfaction: 4.8, uptime: 99.2 },
        { restaurantId: '2', name: 'Fynlo Burger Bar', avgOrderValue: 18.75, transactionCount: 2075, customerSatisfaction: 4.6, uptime: 98.8 },
        { restaurantId: '3', name: 'Fynlo Pizza Palace', avgOrderValue: 25.30, transactionCount: 1296, customerSatisfaction: 4.7, uptime: 97.5 },
        { restaurantId: '4', name: 'Fynlo Taco Stand', avgOrderValue: 8.90, transactionCount: 955, customerSatisfaction: 4.4, uptime: 96.1 },
      ],
      comparisons: [
        { 
          metric: 'Average Order Value', 
          values: [
            { restaurantId: '3', name: 'Fynlo Pizza Palace', value: 25.30 },
            { restaurantId: '2', name: 'Fynlo Burger Bar', value: 18.75 },
            { restaurantId: '1', name: 'Fynlo Coffee Shop', value: 12.50 },
            { restaurantId: '4', name: 'Fynlo Taco Stand', value: 8.90 },
          ]
        },
        { 
