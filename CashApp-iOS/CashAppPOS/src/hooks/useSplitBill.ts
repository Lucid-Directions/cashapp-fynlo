...
export function useSplitBill({
  ...
  const initializeSplit = useCallback((numberOfGroups: number) => {
  ...
  const assignItemToGroup = useCallback((item: EnhancedOrderItem, groupId: string) => {
  ...
  const splitItemAcrossGroups = useCallback((item: EnhancedOrderItem, groupIds: string[]) => {
  ...
  const setGroupCustomAmount = useCallback((groupId: string, amount: number) => {
    setGroups(prev => prev.map(group => 
      group.id === groupId ? { ...group, customAmount: amount } : group
    ));
  }, []);
  
  const setGroupTipPercent = useCallback((groupId: string, percent: number) => {
    setGroups(prev => prev.map(group => 
      group.id === groupId ? { ...group, tipPercent: percent } : group
    ));
  }, []);
  ...
  const toggleGroupTax = useCallback((groupId: string) => {
    setGroups(prev => prev.map(group => 
      group.id === groupId 
        ? { ...group, includeTax: !group.includeTax } 
        : group
    ));
  }, []);
  ...
  const getGroupByItem = useCallback((itemId: string): SplitBillGroup | undefined => {
