...
function convertToEnhancedItem(item: OrderItem | EnhancedOrderItem, userId?: string): EnhancedOrderItem {
    ...
    persist(
      (set, get) => ({
        ...
        addToCart: (newItem) => set((state) => {
          ...
          // Validate the item
          if (!enhancedItem.name || typeof enhancedItem.price !== 'number' || typeof enhancedItem.quantity !== 'number') {
          ...
          const historyEntry: CartHistoryEntry = {
            id: generateUUID(),
            timestamp: new Date().toISOString(),
            action: existingIndex >= 0 ? `Updated ${enhancedItem.name} quantity` : `Added ${enhancedItem.name}`,
            cartBefore: [...state.cart],
            cartAfter: newCart,
        ...
        updateCartItem: (itemId, updates) => set((state) => {
          ...
          if (updates.quantity || updates.price || updates.originalPrice || updates.modifications) {
        ...
        modifyCartItem: (itemId, modifications) => set((state) => {
        ...
        setItemSpecialInstructions: (itemId, instructions) => set((state) => {
          const itemIndex = state.cart.findIndex(item => item.id === itemId);
          if (itemIndex === -1) return state;
        ...
        duplicateCartItem: (itemId) => set((state) => {
        ...
        // Split bill actions
        initializeSplitBill: (splitType, numberOfSplits) => set((state) => {
        ...
        assignItemToSplitGroup: (itemId, groupId) => set((state) => {
        ...
        removeItemFromSplitGroup: (itemId) => set((state) => {
          if (!state.splitBillConfig) return state;
        ...
        updateSplitGroup: (groupId, updates) => set((state) => {
          if (!state.splitBillConfig) return state;
        ...
        completeSplitPayment: (groupId, paymentDetails) => set((state) => {
          if (!state.splitBillConfig) return state;
        ...
        // Template actions
        saveCartAsTemplate: (name, description, tags = []) => set((state) => {
          if (state.cart.length === 0) return state;
        ...
        loadTemplate: (templateId) => set((state) => {
          ...
          const updatedTemplates = state.templates.map(t =>
            t.id === templateId 
              ? { ...t, useCount: t.useCount + 1, lastUsed: now }
              : t
          );
          
          // Update recent templates
          const recentTemplates = [templateId, ...state.recentTemplates.filter(id => id !== templateId)].slice(0, 5);
        ...
        updateTemplateUsage: (templateId) => set((state) => {
          const now = new Date().toISOString();
          return {
        ...
        // Bulk operations
        selectCartItem: (itemId) => set((state) => ({
          selectedItemIds: [...state.selectedItemIds, itemId]
        })),
        
        unselectCartItem: (itemId) => set((state) => ({
        ...
        applyBulkDiscount: (percentage) => set((state) => {
        ...
        // Migration helper
        migrateCartIfNeeded: () => set((state) => {
          if (!cartNeedsMigration(state.cart)) {
            return state;
          }
          
          const migrationResult = migrateCart(
            state.cart as OrderItem[],
            state.user?.id
          );
          ...
          if (migrationResult.errors.length > 0) {
            ...
            ErrorTrackingService.getInstance().trackError(
              new Error('Cart migration failed'),
              { errors: migrationResult.errors, stats: migrationResult.stats }
            );
          }
