 ...
 */
export function formatGroupSummary(group: SplitBillGroup, total: GroupTotal): string {
  const parts: string[] = [];
 ...
 */
export function calculateGroupPercentage(groupTotal: number, billTotal: number): number {
 ...
 */
export function isItemFullyAssigned(item: EnhancedOrderItem, groups: SplitBillGroup[]): boolean {
 ...
 */
export function getGroupsWithItem(itemId: string, groups: SplitBillGroup[]): SplitBillGroup[] {
  return groups.filter((group) => group.items.some((item) => item.originalItemId === itemId));
}
 ...
 */
export function canGroupPay(group: SplitBillGroup, total: GroupTotal): boolean {
...
export function validateSplitBill(
  ...
  const hasAnyAssignment = groups.some((group) => group.items.length > 0 || group.customAmount > 0);
