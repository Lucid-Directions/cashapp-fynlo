...
describe('cartTypeGuards', () => {
  ...
  describe('validateCartIntegrity', () => {
    ...
    it('should detect pricing inconsistencies', () => {
      const cart: EnhancedOrderItem[] = [
        {
          ...
          originalPrice: 4.99,
          modificationPrice: 0.50,
          totalPrice: 15.00, // Should be (4.99 + 0.50) * 2 = 10.98
          modifications: [
            {
              id: 'm1',
              type: 'size',
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Item "Coffee" has inconsistent total price. Expected: 10.98, Actual: 15');
    });
