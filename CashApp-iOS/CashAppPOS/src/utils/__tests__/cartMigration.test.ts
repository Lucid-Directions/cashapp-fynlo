  ...
  describe('migrateCart', () => {
    ...
    it('should handle partial migration with errors', () => {
      ...
      expect(result.warnings).toContain(
        '3 items failed to migrate and were excluded from the cart'
      );
    });

    it('should warn about ID type conversions', () => {
      const cart: OrderItem[] = [{ id: 123, name: 'Coffee', price: 4.99, quantity: 1 }];

      const result = migrateCart(cart);

      expect(result.warnings).toContain(
        'Item "Coffee" ID converted from number (123) to string ("123")'
      );
    });
  ...
  describe('recalculateItemPricing', () => {
    ...
    it('should calculate pricing with modifications', () => {
      const item: EnhancedOrderItem = {
        ...
        totalPrice: 0,
        modifications: [
          { id: 'm1', type: 'size', category: 'Size', name: 'Large', price: 0.5, selected: true },
          {
            id: 'm2',
            type: 'addition',
            category: 'Extras',
      ...
      expect(result.modificationPrice).toBe(2.0); // 0.50 + (0.75 * 2) + 0 (not selected)
