...
export class FynloException extends Error {
  ...
  static unauthorized(message: string = 'Unauthorized', details?: Record<string, any>): FynloException {
  ...
  static tooManyRequests(message: string = 'Too many requests', details?: Record<string, any>): FynloException {
  ...
  static serviceUnavailable(message: string = 'Service unavailable', details?: Record<string, any>): FynloException {
    return new FynloException(message, 503, 'SERVICE_UNAVAILABLE', details);
  }

  static networkError(message: string = 'Network error', details?: Record<string, any>): FynloException {
    return new FynloException(message, 0, 'NETWORK_ERROR', details);
  }

  static offlineError(message: string = 'Device is offline', details?: Record<string, any>): FynloException {
    return new FynloException(message, 0, 'OFFLINE', details);
  }

  static multiTenantViolation(message: string = 'Multi-tenant access violation', details?: Record<string, any>): FynloException {
    return new FynloException(message, 403, 'MULTI_TENANT_VIOLATION', details);
  }

  static encryptionError(message: string = 'Encryption/decryption failed', details?: Record<string, any>): FynloException {
    return new FynloException(message, 500, 'ENCRYPTION_ERROR', details);
  }

  static queueOverflow(message: string = 'Queue size limit exceeded', details?: Record<string, any>): FynloException {
