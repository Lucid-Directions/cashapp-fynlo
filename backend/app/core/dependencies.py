...
from app.core.exceptions import FynloException, AuthorizationException, ResourceNotFoundException
...
async def get_current_user_with_tenant_validation(
    ...
    if restaurant_id:
        ...
        raise AuthorizationException(message="User has no restaurant assigned")
...
async def validate_resource_access(
    ...
    if not resource:
        raise ResourceNotFoundException(message=f"{resource_model.__name__} not found")
    ...
    async def __call__(
        ...
        if TenantSecurity.is_platform_owner(current_user):
            ...
            if restaurant_id and str(restaurant_id) != str(current_user.restaurant_id):
                raise AuthorizationException(message="You can only access your own restaurant")
...
async def platform_owner_required(
    ...
    if not TenantSecurity.is_platform_owner(current_user):
        raise AuthorizationException(message="This endpoint is restricted to platform owners only")
