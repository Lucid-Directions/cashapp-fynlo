...
if "postgresql" in database_url and (
    ...
    if "sslmode" not in database_url:
        ...
        logger.info(
            "Added sslmode=require to DigitalOcean database connection"
        )
...
if "postgresql" in database_url:
    ...
    if ":25060" in database_url or ":25061" in database_url:
        # PgBouncer (port 25061) doesn't support statement_timeout
...
engine = create_engine(
    ...
    # Recycle connections after 1 hour (avoid stale connections)
    pool_recycle=3600,
    # Test connections before using (handles network issues)
    pool_pre_ping=True,
...
class User(Base):
    ...
    )  # Now nullable for Supabase auth
    supabase_id = Column(
...
class Recipe(Base):
    ...
    ingredient = relationship(
        "InventoryItem", back_populates="recipe_ingredients"
    )

    __table_args__ = (
        UniqueConstraint(
            "item_id", "ingredient_sku", name="uq_recipe_item_ingredient"
        ),
    )
...
class InventoryLedgerEntry(Base):
    ...
    sku = Column(
        String(100), ForeignKey("inventory.sku"), nullable=False, index=True
    )
    delta_g = Column(
        Integer, nullable=False
    ...
    source = Column(
        String(50), nullable=False
    )  # E.g., "order_fulfillment", "manual_stock_add", etc.
    source_id = Column(
        String(255), nullable=True
    )  # E.g., order_id, user_id (manual entry), import_batch_id
    ts = Column(
    ...
    inventory_item = relationship(
        "InventoryItem", back_populates="ledger_entries"
    )
