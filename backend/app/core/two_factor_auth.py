...
class TwoFactorAuth:
    ...
    async def confirm_2fa_setup(self, user: User, token: str, setup_key: str) -> bool:
        ...
        """
        if not self.redis:
            raise FynloException(detail="2FA service unavailable", status_code=503)

        # Get setup data
        setup_data = await self.redis.get(setup_key)
        if not setup_data:
            raise ValidationException(detail="2FA setup expired or invalid")
    ...
    async def disable_2fa(self, user: User, current_token: str) -> bool:
        ...
        if not valid:
            raise AuthenticationException(detail="Invalid 2FA token")
