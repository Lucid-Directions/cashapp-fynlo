        ...
        for key, value in os.environ.items():
            ...
            if key in safe_vars:
                ...
                if not any(pattern.match(key) for pattern in cls.SENSITIVE_PATTERNS):
                    ...
                    logger.warning(
                        f"Variable {key} matched sensitive pattern despite being whitelisted"
                    )
    ...
    def sanitize_log_data(cls, data: Any) -> Any:
        ...
        if isinstance(data, dict):
            return {
                key: (
                    "***REDACTED***"
                    if cls.is_sensitive_variable(str(key))
                    else cls.sanitize_log_data(value)
                )
...
class InputValidator:
    ...
    @classmethod
    def sanitize_string(
        cls, value: str, context: str = "general", max_length: int = 1000
    ) -> str:
        """
...
class DeploymentTriggerRequest(BaseModel):
    ...
    reason: str = Field(
        ..., min_length=10, max_length=500, description="Reason for deployment"
    )
...
class MetricsQueryParams(BaseModel):
    ...
    time_range: Optional[str] = Field(
        None, pattern="^(1h|6h|24h|7d|30d)$", description="Time range for metrics"
    )
...
class RequestContext(BaseModel):
    ...
    request_id: str = Field(
        default_factory=lambda: TokenEncryption.generate_secure_token(16)
    )
