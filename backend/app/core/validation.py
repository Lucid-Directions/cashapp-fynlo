...
Provides comprehensive business logic validation with iOS-friendly
error messages.
"""
...
class ValidationResult(BaseModel):
    ...
    def add_error(
        self, field: str, message: str, code: str, value: Any = None
    ):
...
class BusinessValidator:
    ...
    def validate_order_creation(
        ...
        for i, item in enumerate(order_data.get("items", [])):
                ...
                if quantity > available_stock:
                    result.add_error(
                        ...
                        message=(
                            ...
                            f"{available_stock}, Requested: {quantity}"
                        ),
            ...
            if abs(expected_price - item_price) > 0.01:
                result.add_error(
                    field=f"{item_field}.unit_price",
                    message=(
                        f"Price mismatch. Expected: "
                        f"£{expected_price:.2f}, Provided: "
                        f"£{item_price:.2f}"
                    ),
                    code="PRICE_MISMATCH",
        ...
        if order_type not in valid_order_types:
            result.add_error(
                field="order_type",
                message=(
                    f"Invalid order type. Must be one of: "
    ...
    def validate_order_status_transition(
        ...
        if (
            ...
            valid_next = (
                ", ".join(valid_transitions[current_status])
                or "None (final state)"
            )
            result.add_error(
                ...
                message=(
                    ...
                    f"'{new_status}'. Valid transitions: {valid_next}"
                ),
    ...
    def validate_business_hours(
        ...
        day_name = operation_time.strftime("%A").lower()
        ...
        if not (open_time <= current_time <= close_time):
            result.add_error(
                field="operation_time",
                message=(
                    f"Restaurant is closed. Operating hours: "
    ...
    def validate_image_upload(file_data: dict) -> ValidationResult:
        ...
        # Check file size (already validated in file_upload.py,
        # but double-check)
...
def raise_validation_exception(
    ...
    if not validation_result.is_valid:
        error_details = {
            "validation_errors": [
                error.dict() for error in validation_result.errors
            ],
        ...
        if len(validation_result.errors) == 1:
            ...
            user_message = (
                f"{message} ({len(validation_result.errors)} errors found)"
            )
...
def sanitize_string(text: str, max_length: int = 255) -> str:
    """Sanitize string input - delegates to enhanced security module.

    This function is maintained for backward compatibility but now uses
    the enhanced sanitization from app.core.security module.
    """
    # Import here to avoid circular dependencies
    from app.core.security import sanitize_string as enhanced_sanitize

    # Use the enhanced sanitization with strict mode for user input
    return enhanced_sanitize(text, max_length=max_length, strict=True)
...
def validate_model_jsonb_fields(
    model_name: str, field_name: str, data: Any
) -> Any:
