...
class MenuItemResponse:
    ...
    def __init__(self, product, category_name=None):
        ...
        if hasattr(product, 'variants') and product.variants:
            self.variants = [{'id': str(v.id), 'name': v.name, 'price': float(v.price), 'sku': v.sku} for v in product.variants]
    ...
    def dict(self):
        return {'id': self.id, 'name': self.name, 'description': self.description, 'price': self.price, 'category_id': self.category_id, 'category_name': self.category_name, 'is_active': self.is_active, 'sort_order': self.sort_order, 'image_url': self.image_url, 'modifiers': self.modifiers, 'variants': self.variants, 'allergens': self.allergens, 'nutritional_info': self.nutritional_info, 'preparation_time': self.preparation_time, 'created_at': self.created_at, 'updated_at': self.updated_at}
...
class CategoryResponse:
    ...
    def dict(self):
        return {'id': self.id, 'name': self.name, 'description': self.description, 'sort_order': self.sort_order, 'icon': self.icon, 'product_count': self.product_count, 'created_at': self.created_at, 'updated_at': self.updated_at}

@router.get('/menu', response_model=List[dict])
def get_menu_items_optimized(restaurant_id: str=Query(..., description='Restaurant ID'), category: Optional[str]=Query(None, description='Category ID filter'), page: int=Query(1, ge=1), limit: int=Query(50, ge=1, le=100), include_inactive: bool=Query(False), db: Session=Depends(get_db), redis_client: RedisClient=Depends(get_redis), current_user: User=Depends(get_current_user)):
    """
        ...
        if redis_client:
            try:
                ...
                if cached_data:
                    ...
                    return APIResponseHelper.success(data=json.loads(cached_data), message='Menu retrieved from cache')
            except Exception as e:
                logger.warning(f'Redis cache error: {e}')
        query = db.query(Product).options(selectinload(Product.category), selectinload(Product.modifiers), selectinload(Product.variants), selectinload(Product.images)).filter(Product.restaurant_id == restaurant_id)
    ...
    try:
        ...
        if redis_client:
            try:
                cached_data = redis_client.get(cache_key)
                if cached_data:
                    return APIResponseHelper.success(data=json.loads(cached_data), message='Categories retrieved from cache')
            except Exception as e:
                logger.warning(f'Redis cache error: {e}')
        categories_query = db.query(Category, func.count(Product.id).label('product_count')).outerjoin(Product, and_(Product.category_id == Category.id, Product.is_active == True)).filter(Category.restaurant_id == restaurant_id, Category.is_active == True).group_by(Category.id).order_by(Category.sort_order.asc())
        categories_data = categories_query.all()
        response_data = [CategoryResponse(cat, product_count).dict() for (cat, product_count) in categories_data]
        if redis_client and response_data:
            try:
...
def invalidate_menu_cache(restaurant_id: str=Query(...), current_user: User=Depends(get_current_user), redis_client: RedisClient=Depends(get_redis)):
    ...
    try:
        ...
        return APIResponseHelper.success(message=f'Menu cache invalidated ({deleted_count} keys cleared)', meta={'deleted_keys': deleted_count})
