...
def create_platform_default_setting(
    ...
    if current_user.role != 'platform_owner':
        raise AuthenticationException(detail="Only platform owners can create platform default settings", error_code="ACCESS_DENIED")
    
    if setting_data.restaurant_id is not None:
        raise ValidationException(detail="Platform default settings cannot have a restaurant_id.")
    ...
    try:
        ...
        if not created_setting: # Should not happen if service raises error on failure
            raise FynloException(detail="Failed to create platform default setting.")
        ...
        raise ConflictException(detail=f"Failed to create setting: {str(e)}")
...
def update_platform_default_setting(
    ...
    if current_user.role != 'platform_owner':
        raise AuthenticationException(detail="Only platform owners can update platform default settings", error_code="ACCESS_DENIED")
    
    updated_setting = service.update_platform_default_setting(payment_method, updates.dict(exclude_unset=True))
    if not updated_setting:
        raise ResourceNotFoundException(detail=f"Platform default setting for {payment_method.value} not found.")
...
async def list_restaurant_settings(
    ...
    if restaurant_id:
        ...
        if current_restaurant_id and current_restaurant_id != restaurant_id:
            raise AuthenticationException(detail="Cannot access settings for a different restaurant", error_code="ACCESS_DENIED")
...
async def create_or_update_restaurant_setting(
    ...
    # Validate access
    if current_restaurant_id and current_restaurant_id != restaurant_id:
        raise AuthenticationException(detail="Cannot modify settings for a different restaurant", error_code="ACCESS_DENIED")
    ...
    if current_user.role not in ['platform_owner', 'restaurant_owner', 'manager']:
        raise AuthenticationException(detail="Insufficient permissions to modify payment settings", error_code="ACCESS_DENIED")
    ...
    try:
        ...
        if not saved_setting: # Should not happen if service raises specific errors
            raise FynloException(detail="Failed to save restaurant setting.")
        return convert_db_model_to_schema(saved_setting)
    except ValueError as ve:
        raise ValidationException(detail=str(ve))
    except Exception as e: # Catches IntegrityError from service
        raise ConflictException(detail=f"Failed to save setting for restaurant {restaurant_id}: {str(e)}")
...
async def delete_restaurant_setting(
    ...
    # Validate access
    if current_restaurant_id and current_restaurant_id != restaurant_id:
        raise AuthenticationException(detail="Cannot delete settings for a different restaurant", error_code="ACCESS_DENIED")
    ...
    if current_user.role not in ['platform_owner', 'restaurant_owner', 'manager']:
        raise AuthenticationException(detail="Insufficient permissions to delete payment settings", error_code="ACCESS_DENIED")
    
    deleted = service.delete_restaurant_setting(restaurant_id, payment_method)
    if not deleted:
        raise ResourceNotFoundException(detail=f"Setting for restaurant {restaurant_id}, method {payment_method.value} not found.")
