...
async def get_provider_configuration(provider_name: str, current_user: User=Depends(get_current_user)):
    ...
    try:
        ...
        if not config:
            raise ResourceNotFoundException(message="Provider {provider_name} not found", resource_type="Resource")
        provider_config = {'name': config.name, 'enabled': config.enabled, 'environment': config.environment, 'webhook_url': config.webhook_url, 'timeout_seconds': config.timeout_seconds, 'retry_attempts': config.retry_attempts, 'custom_settings': config.custom_settings, 'has_api_key': bool(config.api_key), 'has_secret_key': bool(config.secret_key)}
        return APIResponseHelper.success(data=provider_config, message=f'Configuration for {provider_name} retrieved successfully')
...
async def update_provider_configuration(provider_name: str, config_update: ProviderConfigRequest, current_user: User=Depends(get_current_user)):
    ...
    try:
        ...
        return APIResponseHelper.success(data={'name': updated_config.name, 'enabled': updated_config.enabled, 'environment': updated_config.environment, 'webhook_url': updated_config.webhook_url, 'timeout_seconds': updated_config.timeout_seconds, 'retry_attempts': updated_config.retry_attempts, 'custom_settings': updated_config.custom_settings}, message=f'Configuration for {provider_name} updated successfully')
    except HTTPException:
        raise
    except Exception as e:
        raise FynloException(message='', error_code='INTERNAL_ERROR')
