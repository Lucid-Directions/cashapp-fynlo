...
DEFAULT_SERVICE_CHARGE = {'enabled': True, 'rate': 12.5, 'description': 'Platform service charge', 'currency': 'GBP'}
...
async def get_service_charge_public(db: Session=Depends(get_db)):
    ...
    try:
        ...
        if cached_config:
            ...
            return APIResponseHelper.success(data={'service_charge': cached_config}, message='Service charge configuration retrieved (cached)')
        ...
        try:
            ...
            configs = db.query(PlatformConfiguration).filter(PlatformConfiguration.category == 'service_charge', PlatformConfiguration.is_active == True).limit(4).all()
        ...
        return APIResponseHelper.success(data={'service_charge': result}, message='Service charge configuration retrieved')
    except Exception as e:
        logger.error(f'Error in get_service_charge_public: {e}')
        return APIResponseHelper.success(data={'service_charge': DEFAULT_SERVICE_CHARGE}, message='Service charge configuration (default)')
...
async def get_payment_methods_public():
    ...
    """
    try:
        payment_methods = {'qr_code': {'enabled': True, 'fee_percentage': 1.2, 'name': 'QR Code Payment', 'icon': 'qrcode'}, 'card': {'enabled': True, 'fee_percentage': 2.9, 'name': 'Card Payment', 'icon': 'credit-card'}, 'cash': {'enabled': True, 'fee_percentage': 0.0, 'name': 'Cash', 'icon': 'cash'}, 'apple_pay': {'enabled': True, 'fee_percentage': 2.9, 'name': 'Apple Pay', 'icon': 'apple'}}
