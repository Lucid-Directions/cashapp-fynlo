...
environments. It allows users to associate themselves with restaurants
...
if os.getenv("ENVIRONMENT", "development").lower() == "production":
    raise RuntimeError("Debug endpoints must not be enabled in production!")
...
async def associate_user_with_restaurant(
    ...
    SECURITY WARNING: This endpoint bypasses normal authorization checks.
    It should ONLY be used in development environments for testing.

    The SQL queries use parameterized statements to prevent SQL injection.
    """
    ...
    try:
            ...
            db.execute(
                text(
                    ...
                    INSERT INTO restaurants (
                        ...
                        :id, :name, :legal_name, :address, :city,
                        :postal_code, :country, :phone, :email,
                        :vat_number, :vat_rate, :currency, :timezone,
        ...
        db.execute(
            text(
                "UPDATE users SET restaurant_id = :restaurant_id "
                "WHERE id = :user_id"
            ),
        ...
        return {
            "success": True,
            "message": (
                f"Successfully associated user with restaurant: "
                f"{restaurant_name}"
            ),
