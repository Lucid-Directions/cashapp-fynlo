...
async def get_restaurants(
    ...
    if current_user.role == "platform_owner":
        ...
        if active_only:
            query = query.filter(Restaurant.is_active)
...
async def get_current_restaurant(
    ...
    if not restaurant:
        raise ResourceNotFoundException(
            resource="Restaurant", resource_id=restaurant_id
        )
...
async def create_restaurant(
    ...
    if current_user.role != "platform_owner":
        raise AuthorizationException(
            ...
            details={"required_role": "platform_owner"},
        )
...
async def create_restaurant_onboarding(
    ...
    if current_user.restaurant_id:
        raise ValidationException(
            message="User already has a restaurant associated", field="restaurant_id"
        )
    ...
    if not platform_id:
        ...
        if not default_platform:
            raise ServiceUnavailableError(
                message="No default platform found", service_name="Platform"
            )
    ...
    if current_user.role == "platform_owner":
        if str(restaurant.platform_id) != str(current_user.platform_id):
            raise AuthorizationException(
                ...
                details={"reason": "Restaurant not in user's platform"},
            )
    else:
        ...
        if current_user.role == "manager" and any(
            ...
            raise AuthorizationException(
                ...
                details={"restricted_fields": ["is_active", "payment_methods"]},
            )
...
async def get_platform_stats(
    ...
    if current_user.role != "platform_owner":
        raise AuthorizationException(
            message="Platform owners only", details={"required_role": "platform_owner"}
        )
    platform_id = platform_id or str(current_user.platform_id)
...
async def get_floor_plan(
    ...
    sections_query = (
        db.query(Section)
        .filter(and_(Section.restaurant_id == restaurant_id, Section.is_active))
    ...
    tables_query = (
        ...
        .filter(and_(Table.restaurant_id == restaurant_id, Table.is_active))
    )
