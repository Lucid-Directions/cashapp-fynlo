...
from app.core.exceptions import ValidationException, AuthenticationException, FynloException, ResourceNotFoundException, ConflictException, ServiceUnavailableError, AuthorizationExceptionfrom app.core.validation import (
...
async def create_restaurant_onboarding(
    ...
    if not platform_id:
        ...
        if not default_platform:
            raise ServiceUnavailableError(message="No default platform found", service_name="Platform")        platform_id = str(default_platform.id)
...
async def update_restaurant(
    ...
    if current_user.role == "platform_owner":
        if str(restaurant.platform_id) != str(current_user.platform_id):
            raise AuthorizationException(message="Access denied", details={"reason": "Restaurant not in user's platform"})    else:
