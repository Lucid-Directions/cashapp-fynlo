...
async def create_inventory_item_api(
    ...
    if db_item:
        raise ValidationException(detail=f"Inventory item with SKU {item.sku} already exists.")
...
async def read_inventory_item_api(
    ...
    if db_item is None:
        raise ResourceNotFoundException(detail="Inventory item not found")
...
async def delete_inventory_item_api(
    ...
    if recipes_using_item:
        raise ValidationException(detail="Cannot delete item, it is used in existing recipes.")
...
async def adjust_stock_api(
    ...
    if adjustment.sku != sku:
        raise ValidationException(detail="SKU in path and body do not match.")
...
#     #     raise AuthenticationException(detail="Not enough permissions", error_code="ACCESS_DENIED")
#     # return current_user
...
async def scan_receipt_api(
    ...
    if not response_items:
        raise FynloException(detail="No items could be parsed from the receipt image. Please ensure the image is clear and contains readable text."
        , status_code=422)
