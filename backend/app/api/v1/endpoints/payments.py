...
async def confirm_qr_payment(
    ...
    if not qr_payment_db_record:
        ...
        raise ResourceNotFoundException(
            resource="QR payment", resource_id=qr_payment_id
        )
    ...
    if not order:
        ...
        raise ResourceNotFoundException(
            resource="Associated order", resource_id=qr_payment_db_record.order_id
        )
...
async def process_payment(
    ...
    try:
        ...
        if not order:
            ...
            raise ResourceNotFoundException(
                resource="Order", resource_id=payment_data_req.order_id
            )
        if order.payment_status == "completed":
            await audit_service.create_audit_log(
                event_type=AuditEventType.PAYMENT_FAILURE,
