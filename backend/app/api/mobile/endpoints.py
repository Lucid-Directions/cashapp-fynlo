...
async def get_mobile_menu(
    ...
    try:
        ...
        categories = (
            db.query(Category)
            .filter(Category.restaurant_id == restaurant_id, Category.is_active)
            .all()
        )

        # Get products
        products_query = db.query(Product).filter(
            Product.restaurant_id == restaurant_id, Product.is_active
        )

        if not include_unavailable:
            # Only include available products
            products_query = products_query.filter(Product.is_active)
