...
class CustomerSearchRequest(BaseSearchRequest):
    ...
    ALLOWED_SORT_FIELDS: ClassVar[List[str]] = [
        "created_at",
        "updated_at",
        "total_spent",
        "order_count",
...
class UserSearchRequest(BaseSearchRequest):
    ...
    role: Optional[str] = Field(
        None, pattern="^(platform_owner|restaurant_owner|manager|employee)$"
    )
    restaurant_id: Optional[str] = None
...
class OrderSearchRequest(BaseSearchRequest):
    ...
    status: Optional[str] = Field(
        None, pattern="^(pending|processing|completed|cancelled)$"
    )
    ...
    def validate_max_amount(cls, v, values):
        # Execute validate_max_amount operation
...
class ProductSearchRequest(BaseSearchRequest):
    ...
    def validate_max_price(cls, v, values):
        # Execute validate_max_price operation
...
class BulkOperationRequest(BaseModel):
    ...
    def validate_ids(cls, v):
        # Execute validate_ids operation
        for id_val in v:
