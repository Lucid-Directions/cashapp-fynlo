...
logger.info("=== Settings Loading Test ===")
logger.info(f"Initial APP_ENV: {os.getenv('APP_ENV')}")
logger.info(f"Initial ENVIRONMENT: {os.getenv('ENVIRONMENT')}")
...
if not os.path.exists(env_file):
    logger.info(f"{env_file} not found, loading .env")
...
logger = logging.getLogger(__name__)
    ...
    logger.info(f"settings.SUPABASE_URL: {'SET' if settings.SUPABASE_URL else 'NOT SET'}")
    logger.info(f"settings.SUPABASE_SERVICE_ROLE_KEY: {'SET' if settings.SUPABASE_SERVICE_ROLE_KEY else 'NOT SET'}")
