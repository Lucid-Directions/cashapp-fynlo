...
def suggest_replacement(status_code: int, line: str) -> str:
    ...
    return replacements.get(
        status_code, f'raise FynloException("{detail}", status_code={status_code})'
    )
...
def main():
    ...
    # Report findings
    logger.error(
        f"Found {total_findings} HTTPException usages in {len(files_with_issues)} files\n"
    )
    ...
    for file_path, findings in files_with_issues:
        logger.info(
            f"  - {file_path.relative_to(api_dir.parent.parent)} ({len(findings)} occurrences)"
        )
