...
class FileMigrator:
    ...
    async def _migrate_directory(
        self, local_path: Path, spaces_folder: str
    ) -> List[Dict[str, Any]]:
    ...
    async def _migrate_single_file(
        self, file_path: Path, spaces_folder: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"✅ Successfully migrated: {file_path} → {upload_result['cdn_url']}"
            )
    ...
    def save_migration_report(
        self, results: List[Dict[str, Any]], backup_info: Dict[str, Any]
    ) -> str:
...
async def main():
    ...
    logger.info(
        f"📁 Found {backup_info['total_files']} files ({backup_info['total_size'] / (1024*1024):.1f} MB)"
    )
