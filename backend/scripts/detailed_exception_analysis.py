    ...
    for file, lines in issues.items():
        for line_no, line in lines:
            if "FynloException(" in line:
                categorized["FynloException_code_to_error_code"].append(
                    (file, line_no, line)
                )
            elif "AuthenticationException(" in line:
                categorized["AuthenticationException_remove_code"].append(
                    (file, line_no, line)
                )
            elif "ValidationException(" in line:
                categorized["ValidationException_remove_code"].append(
                    (file, line_no, line)
                )
            elif "ResourceNotFoundException(" in line:
                categorized["ResourceNotFoundException_remove_code"].append(
                    (file, line_no, line)
                )
            elif "BusinessLogicException(" in line:
                categorized["BusinessLogicException_remove_code"].append(
                    (file, line_no, line)
                )
            else:
