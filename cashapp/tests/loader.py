...
from .suite import CashAppSuite
from .result import CashAppTestResult
...
def get_test_modules(module):
    ...
    results = _get_tests_modules(importlib.util.find_spec(f'cashapp.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"cashapp.upgrade.{module}",
        f"cashapp.addons.{module}.migrations",
        f"cashapp.addons.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    return CashAppSuite(sorted(tests, key=lambda t: t.test_sequence))
...
def run_suite(suite, global_report=None):
    ...
    results = CashAppTestResult(global_report=global_report)
    suite(results)
