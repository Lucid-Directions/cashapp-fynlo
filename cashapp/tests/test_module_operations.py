...
import cashapp
from cashapp.tools import config, topological_sort, unique
from cashapp.modules.registry import Registry
from cashapp.netsvc import init_logger
from cashapp.tests import standalone_tests
import cashapp.tests.loader

_logger = logging.getLogger('cashapp.tests.test_module_operations')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of CashApp modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra CashApp modules")
...
def test_standalone(args):
    ...
    for module_name in registry._init_modules:
        ...
        cashapp.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        cashapp.tools.config['addons_path'] = ','.join([args.addons_path, cashapp.tools.config['addons_path']])
        if args.data_dir:
            cashapp.tools.config['data_dir'] = args.data_dir
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'cashapp.modules.loading': {'level': 'CRITICAL'},
            'cashapp.sql_db': {'level': 'CRITICAL'},
            'cashapp.models.unlink': {'level': 'WARNING'},
            'cashapp.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
