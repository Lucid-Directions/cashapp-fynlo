# -*- coding: utf-8 -*-
"""
The module :mod:`cashapp.tests.common` provides unittest test cases and a few
...
import cashapp
from cashapp import api
from cashapp.exceptions import AccessError
from cashapp.fields import Command
from cashapp.http import BadRequest
from cashapp.modules import module
from cashapp.modules.registry import Registry
from cashapp.service import security
from cashapp.sql_db import BaseCursor, Cursor, TestCursor
from cashapp.tools import config, float_compare, mute_logger, profiler, SQL, DotDict
from cashapp.tools.mail import single_email_re
from cashapp.tools.misc import find_in_path, lower_logging
from cashapp.tools.xml_utils import _validate_xml
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 18.0: cashapp.tests.common.Form is deprecated, use cashapp.tests.Form",
...
# The cashapp library is supposed already configured.
ADDONS_PATH = cashapp.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = cashapp.SUPERUSER_ID
...
def get_db_name():
    db = cashapp.tools.config['db_name']
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding CashApp module name.
    """
    def register(func):
        # register func by cashapp module name
        if func.__module__.startswith('cashapp.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most cashapp operations;
    """
    if not login:
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('cashapp.addons.'):
...
class BaseCase(case.TestCase, metaclass=MetaCase):
    """ Subclass of TestCase for CashApp-specific code. This class is abstract and
    expects self.registry, self.cr and self.uid to be initialized by subclasses.
    """

    longMessage = True      # more verbose error message by default: https://www.cashapp.com/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~cashapp.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        request = Mock(
            ...
            env=self.env,
            session=DotDict(cashapp.http.get_default_session(), debug='1'),
        )
        try:
            self.env.flush_all()
            self.env.invalidate_all()
            cashapp.http._request_stack.push(request)
            yield
            self.env.flush_all()
            self.env.invalidate_all()
        finally:
            popped_request = cashapp.http._request_stack.pop()
    ...
    def _patchExecute(self, actual_queries, flush=True):
        ...
        with (
            patch('cashapp.sql_db.Cursor.execute', execute),
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/cashapp/addons/" in filename:
                        filename = filename.rsplit("/cashapp/addons/", 1)[1]
                    if count > expected:
    ...
    def assertRecordValues(
            self,
            records: cashapp.models.BaseModel,
    ...
    def check_test_cursor(self, operation):
        if cashapp.modules.module.current_test != self:
            message = f"Trying to open a test cursor for {self.canonical_tag} while already in a test {cashapp.modules.module.current_test.canonical_tag}"
            _logger.runbot(message)
            raise BadRequest(message)
        request = cashapp.http.request
...
class Approx:  # noqa: PLW1641
    ...
    """
    def __init__(self, value: float, rounding: int | float | cashapp.addons.base.models.res_currency.Currency, /, decorate: bool) -> None:  # noqa: PYI041
        self.value = value
...
class TransactionCase(BaseCase):
    ...
    cr: Cursor = None
    muted_registry_logger = mute_logger(cashapp.modules.registry._logger.name)
    freeze_time = None
    ...
    def _gc_filestore(cls):
        ...
        with Registry(get_db_name()).cursor() as cr:
            gc_env = api.Environment(cr, cashapp.SUPERUSER_ID, {})
    ...
    def setUpClass(cls):
        ...
        cls.env = api.Environment(cls.cr, cashapp.SUPERUSER_ID, {})
        ...
        cls._crypt_context_patcher = patch('cashapp.addons.base.models.res_users.Users._crypt_context', _crypt_context)
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot', date_format="%Y%m%d_%H%M%S_%f"):
    ...
    screenshots_dir = pathlib.Path(cashapp.tools.config['screenshots']) / get_db_name() / 'screenshots'
...
class ChromeBrowser:
    ...
    def __init__(self, test_case: HttpCase, success_signal: str = DEFAULT_SUCCESS_SIGNAL, headless: bool = True, debug: bool = False):
        ...
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_cashapp')

        otc = cashapp.tools.config
...
which leads to stray network requests and inconsistencies."""
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way CashApp widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    @classmethod
    def http_port(cls):
        if cashapp.service.server.server is None:
            return None
        return cashapp.service.server.server.httpd.server_port
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('cashapp.service.http.request.')]
        ...
        if request_threads:
            ...
            cashapp.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        cashapp.http.root.session_store.save(self.session)

    def authenticate(self, user, password, browser: ChromeBrowser = None):
        if getattr(self, 'session', None):
            cashapp.http.root.session_store.delete(self.session)

        self.session = session = cashapp.http.root.session_store.new()
        session.update(cashapp.http.get_default_session(), db=get_db_name())
        session.context['lang'] = cashapp.http.DEFAULT_LANG
        ...
        if user: # if authenticated
            ...
            with patch('cashapp.addons.base.models.res_users.Users._check_credentials', new=patched_check_credentials):
        ...
        cashapp.http.root.session_store.save(session)
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', f"cashapp.startTour({tour_name!r}, {json.dumps(options)})")
        ready = kwargs.pop('ready', f"cashapp.isTourReady({tour_name!r})")
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('cashapp.http.Request._get_profiler_context_manager', route_profiler))
...
def tagged(*tags):
    ...
    By default, all Test classes from cashapp.tests.common have a test_tags
...
class freeze_time:
    """ Object to replace the freezegun in CashApp test suites
