...
class Cursor(BaseCursor):
    ...
    def execute_values(self, query, argslist, template=None, page_size=100, fetch=False):
        ...
        """
        # CashApp Cursor only proxies all methods of psycopg2 Cursor. This is a patch for problems caused by passing
...
def db_connect(to, allow_uri=False, readonly=False):
    ...
    maxconn = cashapp.evented and tools.config['db_maxconn_gevent'] or tools.config['db_maxconn']
...
def close_db(db_name):
    """ You might want to call cashapp.modules.registry.Registry.delete(db_name) along this function."""
    if _Pool:
