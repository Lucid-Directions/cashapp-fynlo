...
"""
Miscellaneous tools used by CashApp.
"""
...
import cashapp
import cashapp.addons
# get_encodings, ustr and exception_to_unicode were originally from tools.misc.
# There are moved to loglevels until we refactor tools.
from cashapp.loglevels import exception_to_unicode, get_encodings, ustr  # noqa: F401
...
if typing.TYPE_CHECKING:
    ...
    from cashapp.api import Environment
    from cashapp.addons.base.models.res_lang import LangData
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if cashapp.tools.config['db_host']:
        env['PGHOST'] = cashapp.tools.config['db_host']
    if cashapp.tools.config['db_port']:
        env['PGPORT'] = str(cashapp.tools.config['db_port'])
    if cashapp.tools.config['db_user']:
        env['PGUSER'] = cashapp.tools.config['db_user']
    if cashapp.tools.config['db_password']:
        env['PGPASSWORD'] = cashapp.tools.config['db_password']
    return env
...
def file_path(file_path: str, filter_ext: tuple[str, ...] = ('',), env: Environment | None = None) -> str:
    ...
    addons_paths = [*cashapp.addons.__path__, root_path, *temporary_paths]
...
def file_open(name: str, mode: str = "r", filter_ext: tuple[str, ...] = (), env: Environment | None = None):
    ...
    Examples::
        ...
        >>> with file_open('/opt/cashapp/addons/hr/static/description/icon.png', 'rb') as f:
        ...     contents = f.read()
...
def file_open_temporary_directory(env: Environment):
    ...
    Examples::
        ...
        >>> with cashapp.tools.file_open_temporary_directory(self.env) as module_dir:
        ...    with zipfile.ZipFile('foo.zip', 'r') as z:
        ...        z.extract('foo/__manifest__.py', module_dir)
        ...    with cashapp.tools.file_open('foo/__manifest__.py', env=self.env) as f:
        ...        manifest = f.read()
...
class mute_logger(logging.Handler):
    ...
    Can be used as context manager or decorator::
        ...
        @mute_logger('cashapp.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('cashapp.foo.bar'):
            do_suff()
    """
...
def format_date(
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = cashapp.fields.Datetime.from_string(value)
            value = cashapp.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = cashapp.fields.Datetime.from_string(value)
...
def format_datetime(
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = cashapp.fields.Datetime.from_string(value)
    else:
        timestamp = value
...
def limited_field_access_token(record, field_name, timestamp=None):
    ...
    :type record: class:`cashapp.models.Model`
