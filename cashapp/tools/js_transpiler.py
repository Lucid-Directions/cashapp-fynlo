"""
This code is what let us use ES6-style modules in cashapp.
Classic CashApp modules are composed of a top-level :samp:`cashapp.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@cashapp-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** cashapp-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual cashapp one with minimal changes.
...
from cashapp.tools.misc import OrderedSet

_logger = logging.getLogger(__name__)

def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom cashapp modules.
    ...
    legacy_cashapp_define = get_aliased_cashapp_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_cashapp_define, module_path, dependencies),
    ]
    for s in steps:
        content = s(content)
    if legacy_cashapp_define:
        content += legacy_cashapp_define
    return content
...
def url_to_module_path(url):
    """
    CashApp modules each have a name. (cashapp.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_cashapp_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the cashapp.define call.
    ...
    """
    return f"""cashapp.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
CASHAPP_MODULE_RE = re.compile(r"""
    \s*                             # starting white space
    \/(\*|\/)                       # /* or //
    .*                              # any comment in between (optional)
    @cashapp-module                    # '@cashapp-module' statement
...
def is_cashapp_module(url, content):
    """
    Detect if the file is a native cashapp module.
    We look for a comment containing @cashapp-module.

    :param url:
    :param content: source code
    :return: is this a cashapp module that need transpilation ?
    """
    result = CASHAPP_MODULE_RE.match(content)
...
def get_aliased_cashapp_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the cashapp source base being:
    ...
    > cashapp.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @cashapp-module */
        // after
        /** @cashapp-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @cashapp-module */
        // after
        /** @cashapp-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = CASHAPP_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\ncashapp.define(`%s`, ['%s'], function (require) {
                        return require('%s');
