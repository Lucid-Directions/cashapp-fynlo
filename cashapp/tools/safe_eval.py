...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False, filename=None):
    ...
    except cashapp.exceptions.UserError:
        raise
    except cashapp.exceptions.RedirectWarning:
        raise
...
`cashapp.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `cashapp.tools.safe_eval`.
""")
    return d
...
from cashapp._monkeypatches.pytz import patch_pytz  # noqa: E402, F401
patch_pytz()
