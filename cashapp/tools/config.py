...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        self.options = {
            'admin_passwd': 'admin',
            'csv_internal_sep': ',',
            'publisher_warranty_url': 'http://services.cashapp.com/publisher-warranty/',
            'reportgz': False,
            'root_path': None,
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.cashapprc (or to ~/.openerp_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store CashApp data")
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'cashapp_tests')
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "cashapp.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="cashapp.http:DEBUG", help='shortcut for --log-handler=cashapp.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="cashapp.sql_db:DEBUG", help='shortcut for --log-handler=cashapp.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate CashApp to another language. "
    ...
    def parse_config(self, args: list[str] | None = None, *, setup_logging: bool | None = None) -> None:
        ...
        This method initializes cashapp.tools.config and openerp.conf (the
        ...
        Typical usage of this method:
            ...
            cashapp.tools.config.parse_config(sys.argv[1:])
        """
        opt = self._parse_config(args)
        if setup_logging is not False:
            cashapp.netsvc.init_logger()
            ...
            if setup_logging is None:
                warnings.warn(
                    "As of CashApp 18, it's recommended to specify whether"
                    " you want CashApp to setup its own logging (or want to"
                    " handle it yourself)",
                    category=PendingDeprecationWarning,
                    stacklevel=2,
                )
        self._warn_deprecated_options()
        cashapp.modules.module.initialize_sys_path()
        return opt
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'cashapp.conf')
        else:
            rcfilepath = os.path.expanduser('~/.cashapprc')
            old_rcfilepath = os.path.expanduser('~/.openerp_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.cashapprc' and '.openerp_serverrc' in your path. Please keep only one of "\
                "them, preferably '.cashapprc'.")
