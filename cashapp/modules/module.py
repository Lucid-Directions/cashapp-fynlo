...
import cashapp
import cashapp.tools as tools
import cashapp.release as release
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `cashapp.upgrade`"""

    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^cashapp\.addons\.base\.maintenance\.migrations\b", fullname):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("cashapp.addons.base.maintenance.migrations", "cashapp.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``cashapp.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook cashapp.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in cashapp.addons.__path__:
        cashapp.addons.__path__.append(dd)

    # hook cashapp.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(ad.strip()))
        if ad not in cashapp.addons.__path__:
            cashapp.addons.__path__.append(ad)

    # hook cashapp.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in cashapp.addons.__path__ and os.path.isdir(base_path):
        cashapp.addons.__path__.append(base_path)

    # hook cashapp.upgrade on upgrade-path
    from cashapp import upgrade
    ...
    # create decrecated module alias from cashapp.addons.base.maintenance.migrations to cashapp.upgrade
    spec = importlib.machinery.ModuleSpec("cashapp.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["cashapp.addons.base.maintenance"] = maintenance_pkg
    sys.modules["cashapp.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to cashapp and "crm"-like to cashapp.addons
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    if re.search(r"[\/\\]", module):
        return False
    for adp in cashapp.addons.__path__:
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    sorted_paths = sorted(cashapp.addons.__path__, key=len, reverse=True)
...
def load_openerp_module(module_name):
    ...
    qualname = f'cashapp.addons.{module_name}'
...
def get_modules():
    ...
    plist = []
    for ad in cashapp.addons.__path__:
