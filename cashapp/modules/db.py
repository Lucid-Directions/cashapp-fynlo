...
def is_initialized(cr):
    ...
    """
    return cashapp.tools.sql.table_exists(cr, 'ir_module_module')
...
def initialize(cr):
    ...
    """
    try:
        f = cashapp.tools.misc.file_path('base/data/base_data.sql')
    except FileNotFoundError:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise IOError(m)

    with cashapp.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())  # pylint: disable=sql-injection

    for i in cashapp.modules.get_modules():
        mod_path = cashapp.modules.get_module_path(i)
        if not mod_path:
            continue

        # This will raise an exception if no/unreadable descriptor file.
        info = cashapp.modules.get_manifest(i)
...
def has_trigram(cr):
    ...
    pg_trgm module but any similar function will be picked by CashApp.
