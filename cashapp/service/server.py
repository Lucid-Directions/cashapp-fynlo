...
from cashapp.release import nt_service_name
from cashapp.tools import config
from cashapp.tools.cache import log_ormcache_stats
from cashapp.tools.misc import stripped_sys_argv, dumpstacks
...
def set_limit_memory_hard():
    ...
    if cashapp.evented and config['limit_memory_hard_gevent']:
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'cashapp.service.http.request.%s' % (me.ident,)
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in cashapp.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="cashapp.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from cashapp.addons.base.models.ir_cron import ir_cron
        def _run_cron(cr):
            pg_conn = cr._cnx
            ...
            while config['limit_time_worker_cron'] <= 0 or (time.monotonic() - alive_time) <= config['limit_time_worker_cron']:
                ...
                registries = cashapp.modules.registry.Registry.registries
        ...
        while True:
            conn = cashapp.sql_db.db_connect('postgres')
    ...
    def cron_spawn(self):
        ...
        for i in range(cashapp.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="cashapp.service.cron.cron%d" % i)
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                from cashapp.tests.result import _logger as logger  # noqa: PLC0415
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('cashapp: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(cashapp.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = cashapp.service.db.list_dbs(True)
        return db_names
    ...
    def process_work(self):
        ...
        if len(db_names):
            ...
            from cashapp.addons import base
    ...
    def start(self):
        ...
        dbconn = cashapp.sql_db.db_connect('postgres')
...
def _reexec(updated_modules=None):
    ...
    if cashapp.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    ...
    from cashapp.tests import loader  # noqa: PLC0415
    from cashapp.tests.suite import CashAppSuite  # noqa: PLC0415
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_enable']:
                from cashapp.tests import loader  # noqa: PLC0415
                t0 = time.time()
                t0_sql = cashapp.sql_db.sql_counter
                ...
                if post_install_suite.has_http_case():
                    with registry.cursor() as cr:
                        env = cashapp.api.Environment(cr, cashapp.SUPERUSER_ID, {})
                ...
                _logger.info("%d post-tests in %.2fs, %s queries",
                             ...
                             time.time() - t0,
                             cashapp.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the cashapp http server and cron processor.
    """
    global server
    ...
    if cashapp.evented:
        server = GeventServer(cashapp.http.root)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(cashapp.http.root)
    else:
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which CashApp is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(cashapp.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not cashapp.evented:
        if inotify:
