...
from cashapp.http import SESSION_LIFETIME
from cashapp.tools import config, lazy_property, mute_logger
from cashapp.tests import get_db_name, tagged
...
class TestHttpSession(TestHttpBase):
    ...
    @mute_logger('cashapp.http')  # greeting_none called ignoring args {'debug'}
    ...
    def test_session1_default_session(self):
        ...
        with patch.object(cashapp.http.root.session_store, 'save') as mock_save:
    ...
    def test_session3_logout_15_0_geoip(self):
        ...
        with self.assertLogs('cashapp.http', level='WARNING') as (_, warnings):
            res = self.multidb_url_open('/test_http/ensure_db', dblist=['db1', 'db2'])

        self.assertEqual(warnings, [
            "WARNING:cashapp.http:Logged into database 'idontexist', but dbfilter rejects it; logging session out.",
        ])
    ...
    @patch("cashapp.http.root.session_store.vacuum")
    ...
    def test_session9_logout(self):
        ...
        self.assertTrue(cashapp.http.root.session_store.get(sid), "session should exist")
        self.url_open('/web/session/logout', allow_redirects=False).raise_for_status()
        self.assertFalse(cashapp.http.root.session_store.get(sid), "session should not exist")
    ...
    def test_session10_explicit_session(self):
        ...
        with self.assertLogs('cashapp.http') as capture:
        ...
        self.assertRegex(capture.output[0],
            r"^WARNING:cashapp.http:<function cashapp\.addons\.\w+\.controllers\.\w+\.logout> "
...
class TestSessionStore(HttpCaseWithUserDemo):
    def setUp(self):
        ...
        self.addCleanup(lazy_property.reset_all, cashapp.http.root)
    ...
    def test01_session_nan(self):
        ...
        with self.assertLogs('cashapp.http', level='WARNING') as logs:
            self.assertEqual(cashapp.http.get_session_max_inactivity(self.env), SESSION_LIFETIME)
            self.assertEqual(logs.output[0], "WARNING:cashapp.http:Invalid value for 'sessions.max_inactivity_seconds', using default value.")
    ...
    def test02_session_lifetime_1week(self):
        ...
        with freeze_time() as freeze:
            ...
            self.env['ir.http']._gc_sessions()
            session_from_store = cashapp.http.root.session_store.get(session.sid)
    ...
    def test04_session_lifetime_nodb(self):
        ...
        with freeze_time() as freeze:
            ...
            self.env['ir.http']._gc_sessions()
            session_from_store = cashapp.http.root.session_store.get(session)
