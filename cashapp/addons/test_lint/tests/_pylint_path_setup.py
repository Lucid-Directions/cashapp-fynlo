...
def register(linter):
    ...
    if addons_path:
        ...
        for p in addons_path.split(os.pathsep):
            ...
            cashapp.addons.__path__.append(normp)

def modpath_from_file(filename, path):
    if filename == "./addons":
        return ['cashapp', 'addons']
...
class AddonsPackageFinder(spec.Finder):
    """Finder which special cases cashapp and cashapp.addons, such that they're
    ...
    def find_module(
        ...
        if modname in ('cashapp', 'cashapp.addons'):
