...
from cashapp.tools.image import image_to_base64
...
class TestIrAttachment(TransactionCaseWithUserDemo):
    ...
    def test_10_image_autoresize(self):
        ...
        with Image.open(os.path.join(dir_path, 'cashapp.jpg'), 'r') as logo:
...
class TestPermissions(TransactionCaseWithUserDemo):
    ...
    def test_read_permission(self):
        ...
        attachment_admin = self.Attachments.with_user(cashapp.SUPERUSER_ID).create({'name': 'foo'})
        ...
        self.assertNotEqual(cashapp.SUPERUSER_ID, admin_user.id)
        attachment_admin.with_user(admin_user).datas

    @mute_logger("cashapp.addons.base.models.ir_rule", "cashapp.models")
