...
from cashapp.tools import pdf
from cashapp.tools.misc import file_open
from cashapp.tools.pdf import reshape_text
import io
...
class TestPdf(TransactionCase):
    ...
    def setUp(self):
        ...
        self.minimal_pdf_reader = pdf.CashAppPdfFileReader(self.minimal_reader_buffer)

    def test_cashapp_pdf_file_reader(self):
        ...
        r = pdf.CashAppPdfFileReader(io.BytesIO(out.getvalue()))
        self.assertEqual(len(list(r.getAttachments())), 1)

    def test_cashapp_pdf_file_writer(self):
        attachments = list(self.minimal_pdf_reader.getAttachments())
        self.assertEqual(len(attachments), 0)
        ...
        for count, (name, data) in enumerate([
            ...
            pdf_writer = pdf.CashAppPdfFileWriter()
    ...
    def test_cashapp_pdf_file_reader_with_owner_encryption(self):
        pdf_writer = pdf.CashAppPdfFileWriter()
        pdf_writer.cloneReaderDocumentRoot(self.minimal_pdf_reader)
        ...
        with io.BytesIO(encrypted_content) as reader_buffer:
            pdf_reader = pdf.CashAppPdfFileReader(reader_buffer)
    ...
    def test_merge_pdf(self):
        ...
        merged_pdf_reader = pdf.CashAppPdfFileReader(merged_reader_buffer)
    ...
    def test_branded_file_writer(self):
        ...
        self.assertEqual(pdf_info['/Producer'], 'CashApp')
        self.assertEqual(pdf_info['/Creator'], 'CashApp')
        reader_buffer.close()
