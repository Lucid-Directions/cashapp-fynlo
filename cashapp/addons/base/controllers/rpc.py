...
import cashapp
from cashapp.http import Controller, route, dispatch_rpc, request, Response
from cashapp.fields import Date, Datetime, Command
from cashapp.tools import lazy
from cashapp.tools.misc import frozendict
...
# User code must use the exceptions defined in ``cashapp.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, cashapp.exceptions.RedirectWarning):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, cashapp.exceptions.AccessError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, cashapp.exceptions.AccessDenied):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, cashapp.exceptions.UserError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, cashapp.exceptions.MissingError):
    ...
    #InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpc.client.Fault(cashapp.tools.exception_to_unicode(e), formatted_info)

    return dumps(fault)


class CashAppMarshaller(xmlrpc.client.Marshaller):
    ...
    # Historically, cashapp is sending binary as base64 string.
...
def dumps(params: list | tuple | xmlrpc.client.Fault) -> str:
    response = CashAppMarshaller(allow_none=False).dumps(params)
    return f"""\
<?xml version="1.0"?>
<methodResponse>
{response}
