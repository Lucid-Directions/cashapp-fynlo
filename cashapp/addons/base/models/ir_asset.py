...
import cashapp
import cashapp.modules.module  # get_manifest, don't from-import it
from cashapp import api, fields, models, tools
from cashapp.tools import misc
from cashapp.tools.constants import ASSET_EXTENSIONS, EXTERNAL_ASSET
...
class IrAsset(models.Model):
    ...
    def _fill_asset_paths(self, bundle, asset_paths, seen, addons, installed, **assets_params):
        ...
        for addon in addons:
            for command in cashapp.modules.module._get_manifest_cached(addon)['assets'].get(bundle, ()):
    ...
    def _topological_sort(self, addons_tuple):
        ...
        def mapper(addon):
            manif = cashapp.modules.module._get_manifest_cached(addon)
    ...
    def _get_installed_addons_list(self):
        ...
        return self.env.registry._init_modules.union(cashapp.conf.server_wide_modules or [])
    ...
    def _get_paths(self, path_def, installed):
        ...
        Ex: ('/base/static/file.js', '/home/user/source/cashapp/cashapp/addons/base/static/file.js', 643636800)
        ...
        addon = path_parts[0]
        addon_manifest = cashapp.modules.module._get_manifest_cached(addon)
