...
def log(logger, level, prefix, msg, depth=None):
    warnings.warn(
        "cashapp.netsvc.log is deprecated starting CashApp 18, use normal logging APIs",
...
class PostgreSQLHandler(logging.Handler):
    ...
    def __init__(self):
        ...
        if tools.config['log_db'] != '%d':
            with contextlib.suppress(Exception), tools.mute_logger('cashapp.sql_db'), sql_db.db_connect(tools.config['log_db'], allow_uri=True).cursor() as cr:
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('cashapp.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='cashapp.tools.image')
    ...
    if os.name == 'posix' and isinstance(handler, logging.StreamHandler) and (is_a_tty(handler.stream) or os.environ.get("CASHAPP_PY_COLORS")):
...
DEFAULT_LOG_CONFIGURATION = [
    'cashapp.http.rpc.request:INFO',
    'cashapp.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['cashapp:DEBUG', 'cashapp.sql_db:INFO', 'cashapp.http.rpc:DEBUG'],
    'debug_rpc': ['cashapp:DEBUG', 'cashapp.sql_db:INFO', 'cashapp.http.rpc.request:DEBUG'],
    'debug': ['cashapp:DEBUG', 'cashapp.sql_db:INFO'],
    'debug_sql': ['cashapp.sql_db:DEBUG'],
    'info': [],
    'runbot': ['cashapp:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['cashapp:WARNING', 'werkzeug:WARNING'],
    'error': ['cashapp:ERROR', 'werkzeug:ERROR'],
    'critical': ['cashapp:CRITICAL', 'werkzeug:CRITICAL'],
}
