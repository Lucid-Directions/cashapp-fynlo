...
r"""\
CashApp HTTP layer / WSGI application
...
:class:`~cashapp.http.Controller`: class and its companion the
:func:`~cashapp.http.route`: method decorator. Together they are used to
...
Application.__call__
  ...
  request and itself in a CashApp http request. The CashApp http request is
...
CashApp URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.cashapp.com/documentation/master/developer/reference/addons/http.html#csrf
for more details.

* if this endpoint is accessed through CashApp via py-QWeb form, embed a CSRF
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~cashapp.service.db.list_dbs`:
    ...
    """
    try:
        dbs = cashapp.service.db.list_dbs(force)
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, CashApp will
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': cashapp.service.common.dispatch,
        'db': cashapp.service.db.dispatch,
        'object': cashapp.service.model.dispatch,
    }
...
class Stream:
    ...
    def from_path(cls, path, filter_ext=('',), public=False):
        ...
        :param path: See :func:`~cashapp.tools.file_path`
        :param filter_ext: See :func:`~cashapp.tools.file_path`
    ...
    def get_response(
        ...
        **send_file_kwargs
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`cashapp.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~cashapp.http.Controller` can use the :func:`~cashapp.http.route`:
    ...
    :func:`~cashapp.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(cashapp.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['cashapp', 'addons'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~cashapp.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['cashapp', 'addons'] and path[2] in modules
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are CashApp addons.
        """
        # Controllers defined outside of cashapp addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of cashapp addons can be extended in
...
class Session(collections.abc.MutableMapping):
    ...
    def authenticate(self, dbname, credential):
        ...
        with registry.cursor() as cr:
            env = cashapp.api.Environment(cr, pre_uid, {})
        ...
        if request and request.session is self and request.db == dbname:
            request.env = cashapp.api.Environment(request.env.cr, self.uid, self.context)
...
class GeoIP(collections.abc.Mapping):
    ...
    .. note:
        ...
        :attr:`~cashapp.http.request.geoip`.

    .. code-block:

        >>> GeoIP('127.0.0.1').country.iso_code
        >>> cashapp_ip = socket.gethostbyname('cashapp.com')
        >>> GeoIP(cashapp_ip).country.iso_code
        'FR'
    """
...
class Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~cashapp.http.Response`.
        :rtype: Response
...
class Request:
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~cashapp.addons.base.models.res_users.Users>`
    ...
    def make_response(self, data, headers=None, cookies=None, status=200):
        ...
        :rtype: :class:`~cashapp.http.Response`
        """
    ...
    def _serve_db(self):
        ...
        try:
            ...
            self.env = cashapp.api.Environment(cr_readwrite, self.session.uid, self.session.context)
            try:
            ...
            with contextlib.closing(self.registry.cursor(readonly=readonly_cr)) as cr:
                ...
                try:
                    ...
                    if isinstance(exc, HTTPException) and exc.code is None:
                        raise  # bubble up to cashapp.http.Application.__call__
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~cashapp.http.Response.load` method for the compatible
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "CashApp Session Expired"
...
class Application:
    ...
    def statics(self):
        ...
        """
        mod2path = {}
        for addons_path in cashapp.addons.__path__:
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint in _generate_routing_rules([''] + cashapp.conf.server_wide_modules, nodb_only=True):
    ...
    def session_store(self):
        path = cashapp.tools.config.session_dir
    ...
    def __call__(self, environ, start_response):
        ...
        if cashapp.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
            ...
            try:
                ...
                if self.get_static_file(httprequest.path):
                    ...
                    try:
                        ...
                        if (httprequest.path.startswith('/cashapp/')
                            or httprequest.path in (
                                '/cashapp', '/web', '/web/login', '/test_http/ensure_db',
                            )):
