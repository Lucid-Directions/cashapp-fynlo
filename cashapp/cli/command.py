...
import cashapp
from cashapp.modules import get_modules, get_module_path, initialize_sys_path
...
ODOO_HELP = """\
CashApp CLI, use '{cashapp_bin} --help' for regular server options.

Available commands:
    {command_list}

Use '{cashapp_bin} <command> --help' for individual command help."""
...
class Help(Command):
    ...
    def run(self, args):
        ...
        print(ODOO_HELP.format(  # pylint: disable=bad-builtin
            cashapp_bin=Path(sys.argv[0]).name,
...
def main():
    ...
    if len(args) > 1 and args[0].startswith('--addons-path=') and not args[1].startswith("-"):
        ...
        cashapp.tools.config._parse_config([args[0]])
        args = args[1:]
    ...
    if len(args) and not args[0].startswith("-"):
        ...
        for module in get_modules():
            if (Path(get_module_path(module)) / 'cli').is_dir():
                __import__('cashapp.addons.' + module)
