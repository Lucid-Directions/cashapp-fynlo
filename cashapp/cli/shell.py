...
class Shell(Command):
    """Start cashapp in an interactive shell"""
    ...
    def init(self, args):
        ...
        cashapp.cli.server.report_configuration()
        cashapp.service.server.start(preload=[], stop=True)
    ...
    def shell(self, dbname):
        ...
        if dbname:
            ...
            with registry.cursor() as cr:
                ...
                ctx = cashapp.api.Environment(cr, uid, {})['res.users'].context_get()
                env = cashapp.api.Environment(cr, uid, ctx)
                local_vars['env'] = env
