...
import { after, before, beforeEach, expect, test } from "@cashapp/hoot";
import { press, queryOne } from "@cashapp/hoot-dom";
import { animationFrame, enableTransitions } from "@cashapp/hoot-mock";
import {
    contains,
    defineModels,
    fields,
...
function setSelectionInHtmlField(selector = "p", fieldName = "body") {
    const anchorNode = queryOne(`[name='${fieldName}'] .cashapp-editor-editable ${selector}`);
...
test("HtmlMail save inline html", async function () {
    ...
    await press("enter");
    expect(".cashapp-editor-editable").toHaveInnerHTML("<h1> first </h1>");
