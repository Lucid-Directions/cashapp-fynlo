...
import { mockDate } from "@cashapp/hoot-mock";
...
test("subscribe to known partner presences", async () => {
    ...
    const expectedPresences = [
        `cashapp-presence-res.partner_${serverState.partnerId}`,
        `cashapp-presence-res.partner_${serverState.cashappbotId}`,
    ];
    ...
    expectedPresences.push(`cashapp-presence-res.partner_${bobPartnerId}`);
    await assertSteps([`subscribe - [${expectedPresences.join(",")}]`]);
});
