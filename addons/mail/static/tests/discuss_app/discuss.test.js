...
import { Deferred, mockDate, tick } from "@cashapp/hoot-mock";
import {
    Command,
    getService,
    makeKwArgs,
...
import { runAllTimers } from "@cashapp/hoot-dom";
...
test("Posting message should transform links.", async () => {
    ...
    await start();
    await openDiscuss(channelId);
    await insertText(".o-mail-Composer-input", "test https://www.cashapp.com/");
    await click(".o-mail-Composer-send:enabled");
    await contains("a[href='https://www.cashapp.com/']");
});
...
test("no out-of-focus notification on receiving self messages in chat", async () => {
    ...
    // simulate receiving a new message of self with cashapp out-of-focused
...
test("out-of-focus notif on needaction message in channel", async () => {
    ...
    // simulate receiving a new needaction message with cashapp out-of-focused
...
test("receive new chat message: out of cashapp focus (notification, chat)", async () => {
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({ name: "Dumbledore" });
    const userId = pyEnv["res.users"].create({ partner_id: partnerId });
    const channelId = pyEnv["discuss.channel"].create({
    ...
    await contains(".o-mail-ChatWindow", { count: 0 });
    await assertSteps(["init_messaging"]);
    // simulate receiving a new message with cashapp out-of-focused
    withUser(userId, () =>
        rpc("/mail/message/post", {
            post_data: {
                body: "New message",
...
test("receive new chat messages: out of cashapp focus (tab title)", async () => {
    let stepCount = 0;
    const pyEnv = await startServer();
    ...
    // simulate receiving a new message in chat 1 with cashapp out-of-focused
    ...
    // simulate receiving a new message in chat 2 with cashapp out-of-focused
    await withUser(bobUserId, () =>
        rpc("/mail/message/post", {
            post_data: { body: "Hello world!", message_type: "comment" },
    ...
    await assertSteps(["set_counters:discuss"]);
    // simulate receiving another new message in chat 2 with cashapp focused
    await withUser(bobUserId, () =>
        rpc("/mail/message/post", {
            post_data: { body: "Hello world!", message_type: "comment" },
            thread_id: channelId_2,
...
test("Partner IM status is displayed as thread icon in top bar of channels of type 'chat'", async () => {
    ...
    pyEnv["discuss.channel"].create([
        ...
        },
        {
            channel_member_ids: [
                Command.create({ partner_id: serverState.partnerId }),
                Command.create({ partner_id: serverState.cashappbotId }),
            ],
            channel_type: "chat",
        },
    ]);
...
test("Chatter notification in messaging menu should open the form view even when discuss app is open", async () => {
    ...
    const messageId = pyEnv["mail.message"].create({
        ...
        author_id: serverState.cashappbotId,
        needaction: true,
        res_id: partnerId,
    });
    pyEnv["mail.notification"].create({
