...
test("media dialog: upload", async function () {
    ...
    const anchorNode = queryOne(".cashapp-editor-editable div.o-paragraph");
...
test("mention a partner", async () => {
    ...
    const anchorNode = queryOne(`[name='body'] .cashapp-editor-editable div.o-paragraph`);
    setSelection({ anchorNode, anchorOffset: 0 });
    ...
    await press("enter");
    expect("[name='body'] .cashapp-editor-editable").toHaveInnerHTML(`
    <div class="o-paragraph">
        <a target="_blank" data-oe-protected="true" contenteditable="false" href="https://www.hoot.test/cashapp/res.partner/17" class="o_mail_redirect" data-oe-id="17" data-oe-model="res.partner">
            @Mitchell Admin
        </a>
    </div>`);
});
...
describe("Remove attachments", () => {
    ...
    test("should remove file from html editor if removed from attachment list", async () => {
        ...
        await waitFor(".cashapp-editor-editable");
        const anchorNode = queryOne(".cashapp-editor-editable div.o-paragraph");
        setSelection({ anchorNode, anchorOffset: 0 });
        ...
        await waitFor(".cashapp-editor-editable .o_file_box:has(a:contains('file.txt'))");
        await contains("[name='attachment_ids'] button:has(i.fa-times)").click();
        await waitForNone("[name='attachment_ids'] a:contains('file.txt')");
        await waitForNone(".cashapp-editor-editable .o_file_box:has(a:contains('file.txt'))");
    });
    ...
    test("should remove image from html editor if removed from attachment list", async () => {
        ...
        await waitFor(".cashapp-editor-editable img[data-attachment-id='1']");
        await contains("[name='attachment_ids'] button:has(i.fa-times)").click();
        await waitForNone("[name='attachment_ids'] a:contains('test.jpg')");
        await waitForNone(".cashapp-editor-editable img[data-attachment-id='1']");
    });
});
