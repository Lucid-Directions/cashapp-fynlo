...
import { mockDate, tick } from "@cashapp/hoot-mock";
import { EventBus } from "@cashapp/owl";
import {
    Command,
    getService,
    patchWithCleanup,
...
test("chat window should not open when receiving a new DM from cashappbot", async () => {
    mockDate("2023-01-03 12:00:00"); // so that it's after last interest (mock server is in 2019 by default!)
    const pyEnv = await startServer();
    const userId = pyEnv["res.users"].create({ partner_id: serverState.cashappbotId });
    const channelId = pyEnv["discuss.channel"].create({
        channel_member_ids: [
            Command.create({
                unpin_dt: "2021-01-01 12:00:00",
