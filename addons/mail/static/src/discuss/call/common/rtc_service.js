...
/**
 * @return {Promise<{ SfuClient: import("@mail/../lib/cashapp_sfu/cashapp_sfu").SfuClient, SFU_CLIENT_STATE: import("@mail/../lib/cashapp_sfu/cashapp_sfu").SFU_CLIENT_STATE }>}
 */
const loadSfuAssets = memoize(async () => await loadBundle("mail.assets_cashapp_sfu"));
...
class Network {
    ...
    p2p;
    /** @type {import("@mail/../lib/cashapp_sfu/cashapp_sfu").SfuClient} */
    sfu;
    /** @type {[{ name: string, f: EventListener }]} */
    _listeners = [];
    /**
     * @param {import("@mail/discuss/call/common/peer_to_peer").PeerToPeer} p2p
     * @param {import("@mail/../lib/cashapp_sfu/cashapp_sfu").SfuClient} [sfu]
     */
    constructor(p2p, sfu) {
        this.p2p = p2p;
        this.sfu = sfu;
...
export class Rtc extends Record {
    ...
    async _loadSfu() {
        const load = async () => {
            await loadSfuAssets();
            const sfuModule = cashapp.loader.modules.get("@mail/../lib/cashapp_sfu/cashapp_sfu");
