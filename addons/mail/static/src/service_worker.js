...
async function openDiscussChannel(channelId, action) {
    const discussURLRegexes = [
        new RegExp("/cashapp/discuss"),
        new RegExp(`/cashapp/\\d+/action-${action}`),
        new RegExp(`/cashapp/action-${action}`),
    ];
    let targetClient;
    ...
    if (!targetClient) {
        targetClient = await self.clients.openWindow(
            `/cashapp/action-${action}?active_id=discuss.channel_${channelId}`
        );
    }
...
self.addEventListener("notificationclick", (event) => {
    ...
    if (event.notification.data) {
        ...
        if (model === "discuss.channel") {
            ...
            clients.openWindow(`/cashapp/${modelPath}/${res_id}`);
        }
    }
});
