...
from cashapp.addons.crm.tests.common import TestCrmCommon
from cashapp.addons.iap.tools import iap_tools
from cashapp.tests.common import tagged, users
...
class TestCRMLead(TestCrmCommon):
    ...
    def setUpClass(cls):
        ...
        cls.emails_provider_generic = {
            ...
            ('fp@cashapp.com', 'fp@cashapp.com'),
            ('fp.alias@mail.cashapp.com', 'fp.alias@mail.cashapp.com'),
        }
        cls.emails_provider_company = {
            ('robert.poilvert@mycompany.com', 'mycompany.com'),
            ('fp@subdomain.cashapp.com', 'subdomain.cashapp.com'),
        }
        ...
        cls.lead_generic_email_dupes = cls.env['crm.lead'].create([
            # email based: normalized version used for email domain criterion
            {
                ...
                'name': 'Dupe1 of fp@cashapp.com (same email)',
                'type': 'lead',
            },
            {
                'email_from': 'FP@cashapp.com',
                'name': 'Dupe2 of fp@cashapp.com (same email)',
                'type': 'lead',
            },
            # phone_sanitized based
            {
                'email_from': 'not.fp@not.cashapp.com',
                'name': 'Dupe3 of fp@cashapp.com (same phone sanitized)',
                'phone': '+1 202 555 0123',
                'type': 'lead',
            },
            {
                ...
                'name': 'Dupe4 of fp@cashapp.com (same phone sanitized)',
                'type': 'lead',
            },
            # same commercial entity
            {
                'name': 'Dupe5 of fp@cashapp.com (same commercial entity)',
                'partner_id': cls.test_partners[1].id,
            },
            {
                'name': 'Dupe6 of fp@cashapp.com (same commercial entity)',
    ...
    def test_assert_initial_values(self):
        ...
        self.assertEqual(lead_generic.email_domain_criterion, 'fp@cashapp.com')
        self.assertEqual(lead_generic.email_normalized, 'fp@cashapp.com')
