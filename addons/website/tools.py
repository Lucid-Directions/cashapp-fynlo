...
import cashapp.http
from cashapp.tools.misc import hmac, DotDict, frozendict
...
def MockRequest(
    ...
    from cashapp.tests.common import HttpCase  # noqa: PLC0415
    ...
    if HttpCase.http_port():
        ...
        base_url = f"http://{HOST}:{cashapp.tools.config['http_port']}"
    request = Mock(
        # request
        httprequest=Mock(
            host='localhost',
            path=path,
            app=cashapp.http.root,
            environ=dict(
                EnvironBuilder(
                    path=path,
                    base_url=base_url,
        ...
        type='http',
        future_response=cashapp.http.FutureResponse(),
        params={},
        redirect=env['ir.http']._redirect,
        session=DotDict(
            cashapp.http.get_default_session(),
        ...
        geoip=cashapp.http.GeoIP('127.0.0.1'),
    ...
    if country_code:
        try:
            request.geoip._city_record = cashapp.http.geoip2.models.City(['en'], country={'iso_code': country_code})
        except TypeError:
            request.geoip._city_record = cashapp.http.geoip2.models.City({'country': {'iso_code': country_code}})
    ...
    #
    #   router = cashapp.http.root.get_db_router()
    ...
    with contextlib.ExitStack() as s:
        ...
        s.callback(cashapp.http._request_stack.pop)
        s.enter_context(patch('cashapp.http.root.get_db_router', router))
