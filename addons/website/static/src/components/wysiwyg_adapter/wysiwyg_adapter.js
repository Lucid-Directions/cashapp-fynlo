...
import { isMediaElement } from '@web_editor/js/editor/cashapp-editor/src/utils/utils';
...
import { Component, onWillStart, useEffect, onWillUnmount } from "@cashapp/owl";
...
export class WysiwygAdapterComponent extends Wysiwyg {
    ...
    async startEdition() {
        ...
        const webEditorFilterMutationRecords = this.cashappEditor.options.filterMutationRecords;
        Object.assign(this.cashappEditor.options, {
            /**
             * @override
             */
        ...
        this.cashappEditor.observerUnactive();
        ...
        // the dirty observer (not to be confused with cashappEditor
        // observer) as the widgets might trigger DOM mutations.
        this._setObserver();
        this.cashappEditor.observerActive();
    }
    _getBannerCommands() {
        return [];
    }
    ...
    async destroy() {
        ...
        const mod = await cashapp.loader.modules.get('@website/snippets/s_social_media/options')[Symbol.for('default')];
        mod.clearDbSocialValuesCache();

        const formOptionsMod = await cashapp.loader.modules.get('@website/snippets/s_website_form/options')[Symbol.for('default')];
    ...
    _setObserver() {
        const processRecords = (records) => {
            records = this.cashappEditor.filterMutationRecords(records);
            ...
            this.cashappEditor.automaticStepSkipStack();
        ...
        this.cashappEditor.addEventListener('observerUnactive', () => {
            if (this.observer) {
                processRecords(this.observer.takeRecords());
                this.observer.disconnect();
            }
        });
        this.cashappEditor.addEventListener('observerActive', this._observe.bind(this));
    }
    /**
     * @private
     */
        ...
        const commands = [
            {
                ...
                isDisabled: () => !this.cashappEditor.isSelectionInBlockRoot(),
                callback: () => {
    ...
    async getSnippetsMenuClass() {
        const snippetsEditor = await cashapp.loader.modules.get('@website/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    async _onSaveRequest(event) {
        ...
        if (event.data.reload || event.data.reloadEditor) {
            ...
            callback = () => redirect(`/cashapp/action-website.website_preview?website_id=${encodeURIComponent(websiteId)}&path=${currentPath}&enable_editor=1`);
