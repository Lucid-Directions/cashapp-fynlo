...
export class WebsitePreview extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            ...
            if (!session.website_bypass_domain_redirect // Used by the Odoo support (bugs to be expected)
                    ...
                    // mysupercompany.cashapp.com explicitly to enter their
                    // backend instead of mysupercompany.be.
                    // Bugs are to be expected while editing/using the website
                    // mysupercompany.be from mysupercompany.cashapp.com though,
                ...
                this.dialogService.add(WebsiteDialog, {
                    ...
                    onClose: () => {
                        window.location.href = `${encodeURI(this.websiteDomain)}/cashapp/action-website.website_preview?path=${encodedPath}&website_id=${encodeURIComponent(this.websiteId)}`;
                    }
                });
            } else {
    ...
    _isTopWindowURL({ host, pathname }) {
        const backendRoutes = ['/web', '/web/session/logout', '/cashapp'];
        return host !== window.location.host
            || (pathname
                && (backendRoutes.includes(pathname)
                    || pathname.startsWith('/@/')
                    || pathname.startsWith('/cashapp/')
    ...
    /**
     * This replaces the browser url (/cashapp/action-website...) with
    ...
    _replaceBrowserUrl() {
        if (!wUtils.isHTTPSorNakedDomainRedirection(this.iframe.el.contentWindow.location.origin, window.location.origin)) {
            ...
            // trigger a traceback.
            history.replaceState(history.state, document.title, '/cashapp');
            return;
        }
    ...
    _onKeydownRefresh(ev) {
        ...
        redirect(`/cashapp/action-website.website_preview?path=${encodeURIComponent(path)}${debugMode}`);
    }
}
