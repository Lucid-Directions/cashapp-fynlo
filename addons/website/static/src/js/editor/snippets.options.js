...
import { Component, markup, useEffect, useRef, useState } from "@cashapp/owl";
                ...
                this.trigger_up('menu_dialog', {
                    ...
                    save: (name, url) => {
                        ...
                        return this.orm.call(
                            ...
                            widget.wysiwyg.cashappEditor.observerUnactive();
                            widget.$target.attr('href', url);
                            $menu.text(name);
                            widget.wysiwyg.cashappEditor.observerActive();
                        });
                    },
                });
                widget.popover.hide();
...
options.registry.Carousel = options.registry.CarouselHandler.extend({
     ...
     */
    async _addSlide() {
        this.options.wysiwyg.cashappEditor.historyPauseSteps();
        ...
        this.options.wysiwyg.cashappEditor.historyUnpauseSteps();
    },
    /**
    ...
    _onControlClick(ev) {
        ...
        this.trigger_up("snippet_edition_request", {exec: async () => {
            ...
            this.options.wysiwyg.cashappEditor.historyUnpauseSteps();
            this.options.wysiwyg.cashappEditor.historyStep();
        }});
    },
    /**
...
options.registry.DeviceVisibility = options.Class.extend({
     ...
     */
    async onTargetHide() {
        this.options.wysiwyg.cashappEditor.observerUnactive("onTargetHide");
        this.$target[0].classList.remove('o_snippet_override_invisible');
        this.options.wysiwyg.cashappEditor.observerActive("onTargetHide");
    },
    /**
     * @override
     */
    async onTargetShow() {
        ...
        if ((this.$target[0].classList.contains('o_snippet_mobile_invisible')
            ...
            this.options.wysiwyg.cashappEditor.observerUnactive("onTargetShow");
            this.$target[0].classList.add('o_snippet_override_invisible');
            this.options.wysiwyg.cashappEditor.observerActive("onTargetShow");
        }
    },
    /**
     * @override
...
options.registry.TextHighlight = options.Class.extend({
    ...
    */
    ...
    _autoAdaptHighlights() {
        this.trigger_up("snippet_edition_request", { exec: async () =>
            await this._refreshPublicWidgets($(this.options.wysiwyg.cashappEditor.editable))
        });
    },
