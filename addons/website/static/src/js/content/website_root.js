...
import "@website/libs/zoomcashapp/zoomcashapp";
    ...
    async _loadGMapAPI(editableMode, refetch) {
        ...
        if (refetch || !this._gmapAPILoading) {
            this._gmapAPILoading = new Promise(async resolve => {
                ...
                window.cashapp_gmap_api_post_load = (async function cashapp_gmap_api_post_load() {
                    ...
                    if (!editableMode && user.isAdmin) {
                        ...
                        this.notification.add(
                            markup(`<div>
                                ...
                                <a href="/cashapp/action-website.action_website_configuration">${urlTitle}</a>
                            </div>`),
                ...
                await loadJS(`https://maps.googleapis.com/maps/api/js?v=3.exp&libraries=places&callback=cashapp_gmap_api_post_load&key=${encodeURIComponent(key)}`);
            });
        }
