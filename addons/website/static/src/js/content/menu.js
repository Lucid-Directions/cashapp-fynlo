...
const BaseAnimatedHeader = animations.Animation.extend({
    ...
    start: function () {
        ...
        // transitionstart, so we listen to a custom cashapp event.
        this._transitionCount = 0;
        this.$el.on('cashapp-transitionstart.BaseAnimatedHeader', () => {
    ...
    _adaptToHeaderChange: function () {
        this.options.wysiwyg && this.options.wysiwyg.cashappEditor.observerUnactive();
        ...
        this.options.wysiwyg && this.options.wysiwyg.cashappEditor.observerActive();
    },
    /**
     * @private
    ...
    _updateHeaderOnScroll: function (scroll) {
        ...
        if (this.headerIsScrolled !== headerIsScrolled) {
            ...
            if (!this.scrollHeightTooShort) {
                ...
                this.$el.trigger('cashapp-transitionstart');
...
publicWidget.registry.StandardAffixedHeader = BaseAnimatedHeader.extend({
     ...
     */
    destroy() {
        this.options.wysiwyg?.cashappEditor.observerUnactive("destroyStandardHeader");
        this.$el.css('transform', '');
        this.el.classList.remove("o_transformed_not_affixed");
        this.options.wysiwyg?.cashappEditor.observerActive("destroyStandardHeader");
        this._super(...arguments);
    },
    ...
    _updateHeaderOnScroll: function (scroll) {
        ...
        this.options.wysiwyg?.cashappEditor.observerUnactive("updateHeaderOnScroll");
        ...
        this.options.wysiwyg?.cashappEditor.observerActive("updateHeaderOnScroll");
    },
});
...
publicWidget.registry.MegaMenuDropdown = publicWidget.Widget.extend({
    ...
    _moveMegaMenu(megaMenuToggleEl) {
        ...
        this.options.wysiwyg?.cashappEditor.observerUnactive("moveMegaMenu");
        ...
        this.options.wysiwyg?.cashappEditor.observerActive("moveMegaMenu");
    },
...
publicWidget.registry.HeaderGeneral = publicWidget.Widget.extend({
     ...
     */
    _onCollapseShow() {
        this.options.wysiwyg?.cashappEditor.observerUnactive("addCollapseClass");
        this.el.classList.add('o_top_menu_collapse_shown');
        this.options.wysiwyg?.cashappEditor.observerActive("addCollapseClass");
    },
    /**
     * @private
     */
    _onCollapseHidden() {
        this.options.wysiwyg?.cashappEditor.observerUnactive("removeCollapseClass");
        ...
        this.options.wysiwyg?.cashappEditor.observerActive("removeCollapseClass");
    },
});
