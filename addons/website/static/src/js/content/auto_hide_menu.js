...
async function autoHideMenu(el, options) {
    ...
    function _adapt() {
        ...
        const cashappEditor = wysiwyg && wysiwyg.cashappEditor;
        if (cashappEditor) {
            cashappEditor.observerUnactive("adapt");
            cashappEditor.withoutRollback(__adapt);
            cashappEditor.observerActive("adapt");
            return;
        }
        __adapt();
    }
