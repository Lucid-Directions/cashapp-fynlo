...
from cashapp import http, models, fields, _
from cashapp.exceptions import AccessError, UserError
from cashapp.http import request, SessionExpiredException
from cashapp.osv import expression
from cashapp.tools import OrderedSet, escape_psql, html_escape as escape, py_to_js_locale
from cashapp.addons.base.models.ir_http import EXTENSION_TO_WEB_MIMETYPES
from cashapp.addons.base.models.ir_qweb import QWebException
from cashapp.addons.portal.controllers.portal import pager as portal_pager
from cashapp.addons.portal.controllers.web import Home
from cashapp.addons.web.controllers.binary import Binary
from cashapp.addons.web.controllers.session import Session
from cashapp.addons.website.tools import get_base_domain
from cashapp.tools.json import scriptsafe as json
...
class Website(Home):
    ...
    def _login_redirect(self, uid, redirect=None):
        ...
        if not redirect and request.params.get('login_success'):
            if request.env['res.users'].browse(uid)._is_internal():
                redirect = '/cashapp?' + request.httprequest.query_string.decode()
            else:
                redirect = '/my'
    ...
    def website_info(self, **kwargs):
        ...
        values = {
            'apps': apps,
            'l10n': l10n,
            'version': cashapp.service.common.exp_version()
        }
    ...
    def website_configurator(self, step=1, **kwargs):
        ...
        action_url = f"/cashapp/action-website.website_configurator?menu_id={request.env.ref('website.menu_website_configuration').id}"
        if step > 1:
    ...
    def pagenew(self, path="", add_menu=False, template=False, redirect=False, **kwargs):
        ...
        if redirect:
            if ext_special_case:  # redirect non html pages to backend to edit
                return request.redirect(f"/cashapp/ir.ui.view/{page.get('view_id')}")
