...
class TestWebsiteAssets(cashapp.tests.HttpCase):
    ...
    def test_01_multi_domain_assets_generation(self):
        ...
        self.url_open(domain_1 + '/cashapp')
        check_asset()
        self.url_open(domain_2 + '/cashapp')
        check_asset()
        self.url_open(domain_1 + '/cashapp')
        check_asset()
        self.url_open(domain_2 + '/cashapp')
...
class TestWebAssets(cashapp.tests.HttpCase):
    def test_assets_url_validation(self):
        website_id = self.env['website'].search([], limit=1, order='id desc').id

        with cashapp.tools.mute_logger('cashapp.addons.web.controllers.binary'):
            self.assertEqual(
