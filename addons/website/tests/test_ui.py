...
from cashapp.addons.web_editor.controllers.main import Web_Editor
from cashapp.fields import Command


@cashapp.tests.tagged('-at_install', 'post_install')
class TestUiCustomizeTheme(cashapp.tests.HttpCase):
...
class TestUiHtmlEditor(HttpCaseWithUserDemo):
    ...
    def test_code_editor_usable(self):
        ...
        url = '/cashapp/action-website.website_preview'
        self.start_tour(url, 'website_code_editor_usable', login='admin')


@cashapp.tests.tagged('external', '-standard', '-at_install', 'post_install')
...
@cashapp.tests.tagged('-at_install', 'post_install')
class TestUiTranslate(cashapp.tests.HttpCase):
...
@cashapp.tests.common.tagged('post_install', '-at_install')
class TestUi(cashapp.tests.HttpCase):

    def test_01_admin_tour_homepage(self):
        self.start_tour("/cashapp", 'homepage', login='admin')
    ...
    def test_10_website_conditional_visibility(self):
        ...
        self.start_tour('/cashapp', 'conditional_visibility_2', login='admin')
    ...
    def test_19_website_page_options(self):
        self.start_tour("/cashapp", "website_page_options", login="admin")
    ...
    def test_21_website_start_cloned_snippet(self):
        self.start_tour('/cashapp', 'website_start_cloned_snippet', login='admin')

    def test_22_website_gray_color_palette(self):
        self.start_tour('/cashapp', 'website_gray_color_palette', login='admin')
    ...
    def test_26_website_media_dialog_icons(self):
        self.env.ref('website.default_website').write({
            'social_twitter': 'https://twitter.com/CashApp',
            'social_facebook': 'https://www.facebook.com/CashApp',
            'social_linkedin': 'https://www.linkedin.com/company/cashapp',
            'social_youtube': 'https://www.youtube.com/user/OpenERPonline',
            'social_github': 'https://github.com/cashapp',
            'social_instagram': 'https://www.instagram.com/explore/tags/cashapp/',
            'social_tiktok': 'https://www.tiktok.com/@cashapp',
        })
        self.start_tour("/", 'website_media_dialog_icons', login='admin')

    def test_27_website_clicks(self):
        self.start_tour('/cashapp', 'website_click_tour', login='admin')
