...
try:
    ...
    from cashapp.tools._vendor.send_file import send_file

import cashapp
import cashapp.modules.registry
from cashapp import SUPERUSER_ID, _, http, api
from cashapp.addons.base.models.assetsbundle import ANY_UNIQUE
from cashapp.exceptions import AccessError, UserError
from cashapp.http import request, Response
from cashapp.tools import file_open, file_path, replace_exceptions, str2bool
from cashapp.tools.image import image_guess_size_from_field_name
from cashapp.tools.mimetypes import guess_mimetype

_logger = logging.getLogger(__name__)

BAD_X_SENDFILE_ERROR = """\
CashApp is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however CashApp is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if cashapp.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=cashapp.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        imgext = '.png'
        dbname = request.db
        uid = (request.session.uid if dbname else None) or cashapp.SUPERUSER_ID
        ...
        if not dbname:
            ...
            try:
                ...
                _logger.warning("While retrieving the company logo, using the CashApp logo instead", exc_info=True)
