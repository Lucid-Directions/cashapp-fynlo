...
export function stateToUrl(state) {
    ...
    const start_url = isScopedApp() ? "scoped_app" : "cashapp";
...
browser.addEventListener("click", (ev) => {
    ...
    if (href && !href.startsWith("#")) {
        ...
        if (
            ...
            browser.location.pathname.startsWith("/cashapp") &&
            (["/web", "/cashapp"].includes(url.pathname) || url.pathname.startsWith("/cashapp/")) &&
            ev.target.target !== "_blank"
        ) {
            ev.preventDefault();
            state = router.urlToState(url);
            if (url.pathname.startsWith("/cashapp") && url.hash) {
