...
import { Component, useRef, useState, markup } from "@cashapp/owl";
...
export const cashappExceptionTitleMap = new Map(
    Object.entries({
        "cashapp.addons.base.models.ir_mail_server.MailDeliveryException": _t("MailDeliveryException"),
        "cashapp.exceptions.AccessDenied": _t("Access Denied"),
        "cashapp.exceptions.MissingError": _t("Missing Record"),
        "cashapp.addons.web.controllers.action.MissingActionError": _t("Missing Action"),
        "cashapp.exceptions.UserError": _t("Invalid Operation"),
        "cashapp.exceptions.ValidationError": _t("Validation Error"),
        "cashapp.exceptions.AccessError": _t("Access Error"),
        "cashapp.exceptions.Warning": _t("Warning"),
    })
);
...
export class RPCErrorDialog extends ErrorDialog {
    ...
    inferTitle() {
        ...
        if (this.props.exceptionName && cashappExceptionTitleMap.has(this.props.exceptionName)) {
            this.title = cashappExceptionTitleMap.get(this.props.exceptionName).toString();
            return;
        }
...
export class WarningDialog extends Component {
    ...
    inferTitle() {
        if (this.props.exceptionName && cashappExceptionTitleMap.has(this.props.exceptionName)) {
            return cashappExceptionTitleMap.get(this.props.exceptionName).toString();
        }
...
registry
    ...
    .add("cashapp.exceptions.AccessDenied", WarningDialog)
    .add("cashapp.exceptions.AccessError", WarningDialog)
    .add("cashapp.exceptions.MissingError", WarningDialog)
    .add("cashapp.addons.web.controllers.action.MissingActionError", WarningDialog)
    .add("cashapp.exceptions.UserError", WarningDialog)
    .add("cashapp.exceptions.ValidationError", WarningDialog)
    .add("cashapp.exceptions.RedirectWarning", RedirectWarningDialog)
    .add("cashapp.http.SessionExpiredException", SessionExpiredDialog)
