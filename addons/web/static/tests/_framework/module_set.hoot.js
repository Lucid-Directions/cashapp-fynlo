...
import { describe, dryRun, globals, start, stop } from "@cashapp/hoot";
import { Deferred } from "@cashapp/hoot-dom";
import { watchKeys, watchListeners } from "@cashapp/hoot-mock";
import { whenReady } from "@cashapp/owl";
...
class ModuleSetLoader extends loader.constructor {
    ...
    constructor(moduleSet) {
        ...
        cashapp.define = this.define.bind(this);
        cashapp.loader = this;
    }
    ...
    cleanup() {
        cashapp.define = define;
        cashapp.loader = loader;
...
const AUTO_INCLUDED_ADDONS = {
    /**
     * spreadsheet addons defines a module that does not starts with `@spreadsheet` but `@cashapp` (`@cashapp/o-spreadsheet)
     * To ensure that this module is loaded, we have to include `cashapp` in the dependencies
     */
    spreadsheet: ["cashapp"],
    /**
...
const CSRF_TOKEN = cashapp.csrf_token;
...
const R_DEFAULT_MODULE = /^@cashapp\/(owl|hoot)/;
