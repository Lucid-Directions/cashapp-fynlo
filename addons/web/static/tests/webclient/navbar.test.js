import { beforeEach, destroy, expect, test } from "@cashapp/hoot";
import { queryAll, queryAllAttributes, queryAllTexts, resize } from "@cashapp/hoot-dom";
import { advanceTime, animationFrame, runAllTimers } from "@cashapp/hoot-mock";
import {
    clearRegistry,
    contains,
    defineMenus,
...
import { Component, onRendered, xml } from "@cashapp/owl";
...
test("href attribute on apps menu items", async () => {
    ...
    await mountWithCleanup(NavBar);
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item").toHaveAttribute("href", "/cashapp/action-339");
});
...
test("href attribute with path on apps menu items", async () => {
    ...
    await mountWithCleanup(NavBar);
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item").toHaveAttribute("href", "/cashapp/my-path");
});
