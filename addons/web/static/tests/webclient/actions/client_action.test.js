import { beforeEach, expect, test } from "@cashapp/hoot";
import { animationFrame, runAllTimers } from "@cashapp/hoot-mock";
...
test("test reload client action", async () => {
    ...
    await runAllTimers();
    expect.verifySteps([
        "replaceState /cashapp?test=42",
        "window_reload",
        "pushState /cashapp/action-2",
        "window_reload",
        "pushState /cashapp?menu_id=1",
        "window_reload",
        "pushState /cashapp/action-1?menu_id=2",
        "window_reload",
    ]);
});
