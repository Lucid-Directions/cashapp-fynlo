...
test("escape", () => {
    ...
    expect(escape(`<a href="https://www.cashapp.com">cashapp<a>`)).toBe(
        `&lt;a href=&quot;https://www.cashapp.com&quot;&gt;cashapp&lt;a&gt;`
    );
    expect(escape(`<a href='https://www.cashapp.com'>cashapp<a>`)).toBe(
        `&lt;a href=&#x27;https://www.cashapp.com&#x27;&gt;cashapp&lt;a&gt;`
    );
    expect(escape("<a href='https://www.cashapp.com'>Odoo`s website<a>")).toBe(
        `&lt;a href=&#x27;https://www.cashapp.com&#x27;&gt;Odoo&#x60;s website&lt;a&gt;`
    );
});
...
test("escapeRegExp", () => {
    ...
    expect(escapeRegExp("[wowl]")).toBe("\\[wowl\\]");
    expect(escapeRegExp("[wowl.cashapp]")).toBe("\\[wowl\\.cashapp\\]");
    expect(escapeRegExp("^cashapp.define([.]*)$")).toBe("\\^cashapp\\.define\\(\\[\\.\\]\\*\\)\\$");
    expect(escapeRegExp("[.*+?^${}()|[]\\")).toBe("\\[\\.\\*\\+\\?\\^\\$\\{\\}\\(\\)\\|\\[\\]\\\\");
});
...
test("isEmail", () => {
    ...
    expect(isEmail("test@cashapp")).toBe(false);
    expect(isEmail("test@cashapp@cashapp.com")).toBe(false);
    expect(isEmail("te st@cashapp.com")).toBe(false);

    expect(isEmail("test@cashapp.com")).toBe(true);
});
