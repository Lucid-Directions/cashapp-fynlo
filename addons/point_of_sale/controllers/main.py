...
from cashapp import http, _
from cashapp.http import request
from cashapp.osv.expression import AND
from cashapp.tools import format_amount
from cashapp.addons.account.controllers.portal import PortalAccount
...
class PosController(PortalAccount):
    ...
    def pos_web(self, config_id=False, from_backend=False, **k):
        ...
        if not pos_config or not pos_config.active or pos_config.has_active_session and not pos_session:
            return request.redirect('/cashapp/action-point_of_sale.action_client_pos_menu')
