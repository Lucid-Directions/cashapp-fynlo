...
import { cashappExceptionTitleMap, ErrorDialog } from "@web/core/errors/error_dialogs";
...
export function handleRPCError(error, dialog) {
    const { data } = error;
    if (cashappExceptionTitleMap.has(error.exceptionName)) {
        const title = cashappExceptionTitleMap.get(error.exceptionName).toString();
