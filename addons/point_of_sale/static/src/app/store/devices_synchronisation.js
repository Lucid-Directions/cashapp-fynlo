...
export default class DevicesSynchronisation {
    ...
    async dispatch(data) {
        ...
        await this.pos.data.call("pos.config", "notify_synchronisation", [
            cashapp.pos_config_id,
            cashapp.pos_session_id,
            cashapp.login_number,
            recordIds,
        ]);
    }
    ...
    async collect(data) {
        ...
        if (cashapp.debug === "assets") {
            console.info("Incoming synchronisation", data);
            console.info("Login number", cashapp.login_number, login_number);
            console.info("Session Ids", cashapp.pos_session_id, session_id);
        }

        if (login_number == cashapp.login_number || session_id != cashapp.pos_session_id) {
            return;
        }
