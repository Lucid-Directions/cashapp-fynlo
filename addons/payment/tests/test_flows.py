...
from cashapp.tests import tagged, JsonRpcException
from cashapp.tools import mute_logger

from cashapp.addons.payment.controllers.portal import PaymentPortal
from cashapp.addons.payment.tests.http_common import PaymentHttpCommon
...
class TestFlows(PaymentHttpCommon):
    ...
    def _test_flow(self, flow):
        ...
        with mute_logger('cashapp.addons.payment.models.payment_transaction'):
    ...
    def test_transaction_wrong_flow(self):
        ...
        with mute_logger("cashapp.http"), self.assertRaises(
            JsonRpcException,
            msg='cashapp.exceptions.UserError: The payment should either be direct, with redirection, or made by a token.',
        ):
    ...
    def test_transaction_route_rejects_unexpected_kwarg(self):
        ...
        with self.assertRaises(JsonRpcException, msg='cashapp.exceptions.ValidationError'):
    ...
    def test_transaction_wrong_token(self):
        ...
        with mute_logger('cashapp.http'), self.assertRaises(JsonRpcException, msg='cashapp.exceptions.ValidationError: The access token is invalid.'):
    ...
    @mute_logger('cashapp.addons.payment.models.payment_transaction')
    def test_direct_payment_triggers_no_payment_request(self):
        self.authenticate(self.portal_user.login, self.portal_user.login)
        self.partner = self.portal_partner
        self.user = self.portal_user
        with patch(
            'cashapp.addons.payment.models.payment_transaction.PaymentTransaction'
